...
export interface DesignField {key:string;label:string;group:string;default:number;min:number;max:number;options?:number[];type?:string;allowAuto?:boolean;unit?:string}
...
export function validateDesign(value:unknown):Designs {
 ...
 for(const [module,fields] of Object.entries(value)){
  ...
  for(const [key,n] of Object.entries(fields)){
   ...
   if(!field||typeof n!=='number'||!Number.isInteger(n)||(!(field.allowAuto&&n===0)&&n<field.min)||n>field.max||field.options&&!field.options.includes(n))throw Error(`Invalid design value: ${module}.${key}`)
