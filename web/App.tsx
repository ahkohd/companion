...
import { Activity, Bell, ArrowDown, ArrowLeft, ArrowRight, ArrowUp, ArrowUpRight, Check, ChevronDown, ChevronRight, CircleHelp, Clock, Cpu, Download, Eye, LayoutDashboard, LayoutGrid, Mail, Monitor, Music2, Volume2, Moon, Sun, MousePointer2, Play, Plus, Radio, RefreshCw, RotateCcw, Search, Settings2, Shapes, SlidersHorizontal, Smile, Sparkles, Unplug, Wifi, X, type LucideIcon } from 'lucide-react'
...
const moduleIcons = {face:Smile,usage:Activity,hey:Mail,clock:Clock,roon:Music2,audio:Volume2,speedDial:LayoutGrid}
...
function Thumbnail({id,className=''}:{id:string|null;className?:string}) {
  return <div className={`animation-thumb ${className}`} aria-hidden="true"><div className={`mini-eyes mini-${id||'idle'}`}><i /><i /></div></div>
}
...
function Overview({snapshot:s,pending,action,navigate}:PageProps&{navigate:(page:Page)=>void}) {
  ...
  return <>
    ...
    <Panel title="More than a face" description="Bring the rest of your day into view." className="module-shortcuts">{(['usage','hey','clock','roon','audio','speedDial'] as const).map(id=>{
      ...
      return <button className="shortcut-row" key={id} onClick={()=>navigate('modules')}>
        ...
        <span><strong>{moduleNames[id]}</strong><small>{id==='usage'?'Your AI usage, at a glance':id==='hey'?'A quieter view of your inbox':id==='roon'?'Your music, within reach':id==='speedDial'?'Your everyday actions, a tap away':id==='audio'?'Sound, at your fingertips':'A little space for the time'}</small></span>
        {id==='speedDial'?<Badge variant="secondary">Built in</Badge>:id==='audio'?<Badge variant="secondary">Mac audio</Badge>:id==='clock'?<Badge variant="secondary">Built in</Badge>:<SourceBadge source={s.modules[id]} enabled={s.settings.modules[id].enabled} network={id==='roon'}/>}
    ...
    <div className="module-cards">{order.map((id,index)=>{
      ...
      return <Panel className={`module-card ${config.enabled?'enabled':''}`} key={id}>
        ...
        <div className="module-meta">
          {source?<SourceBadge source={source} enabled={config.enabled} network={id==='roon'}/>:id==='speedDial'?<Badge variant="secondary">{config.enabled?'Ready':'Off'}</Badge>:id==='audio'?<Badge variant="secondary" className={config.enabled&&s.modules.audio?.status==='ready'?'is-good':''}>{!config.enabled?'Off':s.modules.audio?.status==='ready'?'Connected':'Unavailable'}</Badge>:id==='clock'?<Badge variant="secondary">Computer time</Badge>:<Badge variant="secondary" className={s.connected?'is-good':''}><StatusDot state={s.connected?'connected':'idle'}/>{s.connected?'Herdr connected':'Waiting for Herdr'}</Badge>}
          <span className="module-meta-note">{id==='speedDial'?`${s.settings.modules.speedDial.buttons.length} buttons`:id==='audio'?(s.modules.audio?.deviceName||'Mac input and output'):id==='roon'?(s.modules.roon?.playerName||'Spotify, Apple Music, Roon and macOS'):source?.version?`CLI ${source.version}`:id==='face'?'Live agent states':id==='clock'?'Updates automatically':source?.installed===true?'Detected on this computer':source?.installed===false?'Install the CLI to connect':'Checking this computer'}</span>
