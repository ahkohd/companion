...
const navigation: {id:Page;label:string;icon:LucideIcon;description:string}[] = [
  ...
  {id:'app-settings',label:'Settings',icon:Settings2,description:'Startup, updates, and information about Companion.'},
]
...
function DeviceSettings({snapshot:s,pending,save,action}:PageProps) {
  ...
  return <>
    <Panel className="hardware-card"><div className="hardware-summary"><span className="hardware-icon"><Cpu size={30}/></span><div><Eyebrow>Your companion</Eyebrow><h2>{s.device.profile?.name || 'Waveshare AMOLED'}</h2><p>{s.device.profile ? `${s.device.profile.display.width} x ${s.device.profile.display.height} ${s.device.profile.display.shape} display - ${s.device.profile.status}` : 'ESP32-S3 / 1.75-inch round touch display'}</p><div className="hardware-status"><StatusDot state={s.device.status==='connected'?'done':'idle'}/>{s.device.status==='connected'?'Connected over USB':s.device.status==='disabled'?'USB connection not configured':'Waiting for your device'}</div></div></div><SerialConnection snapshot={s} pending={pending} action={action}/></Panel>
