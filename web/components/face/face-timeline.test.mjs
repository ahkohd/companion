...
before(async () => {
  const result = await build({
    ...
    build: {
      write: false,
      minify: false,
      lib: { entry: 'web/components/face/face-timeline.ts', formats: ['es'] },
    },
  })
...
test('a restarted host preserves the current gaze and expression transition', () => {
  ...
  for (const key of ['x', 'y', 'mix']) {
    near(after.gaze[key], before.gaze[key])
  }
  ...
  for (const key of ['yaw', 'pitch', 'roll']) {
    near(rebased.gaze[key], profile.gaze[key])
  }

  rebased.eyes.forEach((eye, index) => {
    for (const key of ['w', 'h', 'open', 'tilt']) {
      near(eye[key], profile.eyes[index][key])
    }
  })
})
...
test('stale host snapshots cannot move the local clock backwards', () => {
  ...
  const next = timeline.update(
    { ...start, animationMs: 100_020, ageMs: 4020 },
    false,
    1050,
    200_050,
  )
...
test('enabling reduced motion settles an ongoing gaze', () => {
  ...
  assert.ok(moving.gaze.x > 0.2 && moving.gaze.x < 0.35)
...
test('replaying an expression resets its age and preserves the pointer glide', () => {
  const start = input({ state: 'done', look: { x: 0.8, y: -0.4 } })
  const timeline = new FaceTimeline(start, false, 1000, 200_000)
  const before = timeline.tick(1200, 200_200)

  const replay = timeline.update(
    { ...start, animationMs: 100_200, ageMs: 0, changedAt: 200_200 },
    false,
    1200,
    200_200,
  )
...
test('unknown states and invalid initial timestamps still render finite eyes', () => {
  const timeline = new FaceTimeline(
    input({ state: 'not-a-state', animationMs: NaN, ageMs: NaN }),
    false,
    1000,
    200_000,
  )
...
test('changing face size leaves age, gaze and expression timing unchanged', () => {
  for (const reduced of [false, true]) {
    const props = input({ state: 'working', look: { x: 0.4, y: -0.2 }, faceScale: 100 })
    ...
    for (const faceScale of [50, 150, 100]) {
      ...
      near(after.age, before.age)
      near(after.time, before.time)

      assert.deepEqual(after.gaze, before.gaze)
