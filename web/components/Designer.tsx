...
import { Activity, Check, CircleHelp, Clock, Download, Grid2X2, Mail, Music2, Volume2, RotateCcw, Search, SlidersHorizontal, Smile, Undo2, Upload, Wifi, WifiOff } from 'lucide-react'
...
const modules:ModuleId[]=['face','usage','hey','clock','roon','audio']
const icons={face:Smile,usage:Activity,hey:Mail,clock:Clock,roon:Music2,audio:Volume2}
...
export default function Designer({snapshot:s,online,pending,action}:{snapshot:StudioSnapshot;online:boolean;pending:boolean;action:(path:string,payload:unknown,message?:string)=>Promise<boolean>}){
 ...
 if(sampleContent){
  preview.display=module==='face'?{state:'working',label:'3 Working',name:'Building something good',nameShimmer:true}:module==='usage'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'CodexBar',detail:'',pageIndex:0,pageCount:1,primary:{provider:'Codex',label:'Session',remaining:73,reset:'Resets in 2h 14m'},secondary:{provider:'Claude',label:'Weekly',remaining:42,reset:'Resets in 4d 6h'}}}:module==='roon'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'Now Playing',player:'roon',playerName:'Roon',detail:'',track:'A little music',artist:'Your favourite artist',playing:true,canPrevious:true,canNext:true}}:module==='audio'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'Output',detail:'',scope:'output',deviceId:1,deviceName:'Mac speakers',volume:65,muted:false,canVolume:true,canMute:true,pageIndex:0,pageCount:2}}:module==='clock'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'Clock',detail:'',blinkSeparator:s.settings.modules.clock.blinkSeparator,time:s.settings.modules.clock.hourFormat==='24'?'17:20':'5:20',weekday:s.settings.modules.clock.showWeekday?'Wed':''}}:{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'HEY',detail:'',pageIndex:0,pageCount:1,items:[{sender:'Alex Morgan',subject:'A few thoughts on the new companion'},{sender:'Studio team',subject:'Your design review is ready for Friday'},{sender:'Sam Taylor',subject:'See you tomorrow'}].slice(0,selected.rows)}}
 }
