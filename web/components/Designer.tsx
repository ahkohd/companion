...
import { Activity, Check, CircleHelp, Clock, Download, Grid2X2, LayoutGrid, Mail, Music2, Volume2, RotateCcw, Search, SlidersHorizontal, Smile, Undo2, Upload, Wifi, WifiOff } from 'lucide-react'
...
const icons={face:Smile,usage:Activity,hey:Mail,clock:Clock,roon:Music2,audio:Volume2,speedDial:LayoutGrid}
...
export default function Designer({snapshot:s,online,pending,action}:{snapshot:StudioSnapshot;online:boolean;pending:boolean;action:(path:string,payload:unknown,message?:string)=>Promise<boolean>}){
 ...
 const dialConfig={...s.settings.modules.speedDial,screenShape:s.device.profile?.display.shape??'round'}
 const dialPageSize=speedDialPageSize(dialConfig,draft.speedDial)
 const dialPageCount=Math.max(1,Math.ceil(dialConfig.buttons.filter(button=>button.enabled).length/dialPageSize))
 const dialButtons=dialConfig.buttons.filter(button=>button.enabled).slice(0,dialPageSize).map((button,index)=>({id:button.id,label:button.label,color:button.color,iconId:button.icon.assetId,iconIndex:index,status:'idle' as const,enabled:true}))
 const dialSamples=dialButtons.length?dialButtons:['Focus','Browser','Terminal','Music','Notes','Shortcuts','Messages','Calendar','Files','Mail','Lock'].slice(0,Math.min(dialPageSize,SPEED_DIAL_MAX_SLOTS)).map((label,index)=>({id:`sample-${index}`,label,color:null,iconId:'',iconIndex:index,status:'idle' as const,enabled:true}))
 const sampleContent=sample||module==='speedDial'&&!equal(draft.speedDial,s.settings.design?.speedDial)||module==='hey'&&draft.hey.rows!==s.settings.design?.hey.rows||s.module!==module||module!=='face'&&s.display.dashboard?.status!=='ready'
 if(sampleContent){
  preview.display=module==='face'?{state:'working',label:'3 Working',name:'Building something good',nameShimmer:true}:module==='usage'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'CodexBar',detail:'',pageIndex:0,pageCount:1,primary:{provider:'Codex',label:'Session',remaining:73,reset:'Resets in 2h 14m'},secondary:{provider:'Claude',label:'Weekly',remaining:42,reset:'Resets in 4d 6h'}}}:module==='roon'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'Now Playing',player:'roon',playerName:'Roon',detail:'',track:'A little music',artist:'Your favourite artist',playing:true,canPrevious:true,canNext:true}}:module==='audio'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'Output',detail:'',scope:'output',deviceId:1,deviceName:'Mac speakers',volume:65,muted:false,canVolume:true,canMute:true,pageIndex:0,pageCount:2}}:module==='speedDial'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'Speed Dial',detail:'Add buttons in Modules',screenShape:dialConfig.screenShape,layout:s.settings.modules.speedDial.layout,gridSize:s.settings.modules.speedDial.gridSize,listRows:s.settings.modules.speedDial.listRows,showLabels:s.settings.modules.speedDial.showLabels,pageIndex:0,pageCount:dialPageCount,buttons:dialSamples}}:module==='clock'?{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'Clock',detail:'',blinkSeparator:s.settings.modules.clock.blinkSeparator,time:s.settings.modules.clock.hourFormat==='24'?'17:20':'5:20',weekday:s.settings.modules.clock.showWeekday?'Wed':''}}:{state:'idle',label:'',name:'',dashboard:{status:'ready',title:'HEY',detail:'',pageIndex:0,pageCount:1,items:[{sender:'Alex Morgan',subject:'A few thoughts on the new companion'},{sender:'Studio team',subject:'Your design review is ready for Friday'},{sender:'Sam Taylor',subject:'See you tomorrow'}].slice(0,selected.rows)}}
 }
 ...
 return <section className="designer" aria-label="Module designer">
  ...
  <div className="designer-workspace">
   ...
   <div className="designer-canvas">
    ...
    <div className={`designer-stage ${guides?'with-guides':''}`}><DevicePreview snapshot={preview} online={true} pending={pending||!online} onModule={id=>void choose(id)} onLive={()=>setSample(false)} onUsagePage={direction=>void action('usage/page',{direction})} onHeyPage={direction=>void action('hey/page',{direction})} onOpenCard={sampleContent?undefined:request=>void action('open-card',request)} onRoonControl={sampleContent?undefined:(control,player)=>void action('roon/control',{action:control,player})} onRoonPage={sampleContent?undefined:direction=>void action('roon/player',{direction})} onRoonView={sampleContent?undefined:expanded=>void action('roon/view',{expanded})} onAudioControl={sampleContent?undefined:request=>void action('audio/control',request)} onAudioView={sampleContent?undefined:request=>void action('audio/view',request)} onAudioPage={sampleContent?undefined:direction=>void action('audio/page',{direction})} onSpeedDialRun={sampleContent?undefined:request=>void action('speed-dial/run',request)} onSpeedDialPage={sampleContent?undefined:direction=>void action('speed-dial/page',{direction})}/></div>
   ...
   <aside className="designer-inspector" aria-label={`${moduleNames[module]} properties`}><div className="designer-inspector-header"><div><SlidersHorizontal size={15}/><h2>Properties</h2>{changed>0&&<span>{changed} changed</span>}</div><div className="designer-search"><Search size={14}/><Input placeholder="Find a property" aria-label="Find a design property" value={search} onChange={e=>setSearch(e.target.value)}/></div></div><div className="designer-property-list">{!search&&<section className="designer-group"><h3>Display</h3><a className="designer-related-link" href="#device">Edit device colours</a>{(module==='usage'||module==='hey')&&<label className="designer-display-switch">Card backgrounds<Switch checked={s.settings.device.showCardBackgrounds} disabled={!online} aria-label="Designer card backgrounds" onCheckedChange={value=>void action('settings',{device:{showCardBackgrounds:value}})}/></label>}{module==='clock'&&<><label className="designer-display-switch">24-hour time<Switch checked={s.settings.modules.clock.hourFormat==='24'} disabled={!online} aria-label="Designer 24-hour time" onCheckedChange={value=>void action('settings',{modules:{clock:{hourFormat:value?'24':'12'}}})}/></label><label className="designer-display-switch">Show weekday<Switch checked={s.settings.modules.clock.showWeekday} disabled={!online} aria-label="Designer show weekday" onCheckedChange={value=>void action('settings',{modules:{clock:{showWeekday:value}}})}/></label><label className="designer-display-switch">Blink separator<Switch checked={s.settings.modules.clock.blinkSeparator} disabled={!online} aria-label="Designer blink separator" onCheckedChange={value=>void action('settings',{modules:{clock:{blinkSeparator:value}}})}/></label></>}{module==='speedDial'&&<a className="designer-related-link" href="#modules">Edit buttons and layout</a>}{module==='roon'&&<a className="designer-related-link" href="#modules">Choose music players</a>}{module==='face'&&<a className="designer-related-link" href="#animations">Edit animation mappings</a>}</section>}{groups.map(group=><section className="designer-group" key={group}><h3>{group}</h3>{fields.filter(f=>f.group===group).map(field=><DesignControl key={`${module}-${field.key}`} field={field} autoValue={s.settings.device.showCardBackgrounds?44:56} value={selected[field.key]} onChange={value=>change(field.key,value)}/>)}</section>)}{!fields.length&&<p className="designer-no-results">No matching properties.</p>}</div><div className="designer-inspector-foot">{designSchema[module].filter(field=>field.type!=='color').length} controls <span>Device fonts: Geist and Geist Pixel Circle</span></div></aside>
  </div>
 </section>
}
