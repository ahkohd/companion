...
export default function RoonSettings({snapshot:s,pending,save,action}:{snapshot:StudioSnapshot;pending:boolean;save:(patch:unknown,message?:string)=>Promise<boolean>;action:(path:string,payload:unknown,message?:string)=>Promise<boolean>}) {
  ...
  return <div className="roon-settings">
    ...
    <p className="music-navigation-note">Swipe up or down to switch players. Swipe left or right to switch modules. Tap the app icon on the artwork to open its app. macOS may show the same track as Spotify or Apple Music.</p>
    ...
    {ready&&<div className="roon-now-playing">
      <div className="roon-art">{source.artId&&<img src={`/api/roon/art/${source.artId}`} alt="Album artwork"/>}{player!=='system'&&<button key={player} className="music-art-badge" type="button" aria-label={`Open ${playerName}`} title={`Open ${playerName}`} disabled={pending} onClick={()=>control('open')}><PlayerIcon player={player} size={28}/></button>}{source.canLike&&<button className="music-art-like" aria-label={source.liked?'Remove from favourites':'Add to favourites'} aria-pressed={!!source.liked} disabled={pending} onClick={()=>control('like')}><Heart size={18} fill={source.liked?'currentColor':'none'}/></button>}</div>
