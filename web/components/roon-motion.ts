...
type Timeline={
  motion:Motion;target:number;previous:number;spinning:boolean
...
function sample(state:Timeline,now:number) {
  ...
  if(state.geometry){
    const from=state.geometry,elapsed=now-from.at
    ...
    if(elapsed>=280)state.geometry=null
  }
  if(state.orientation){
    const from=state.orientation,elapsed=now-from.at
    angle=from.angle*(1-ease(elapsed/280))
    if(elapsed>=280)state.orientation=null
  }else if(state.spinning){
    // Avoid a large visual jump when a background tab resumes rendering.
    angle=(angle+Math.min(100,Math.max(0,now-state.previous))*.018+360)%360
  }
  state.previous=now
...
export function useRoonMotion(expanded:boolean,playing:boolean,animate:boolean,spin:boolean,reduced:boolean) {
  const timeline=useRef<Timeline>({motion:{progress:expanded?1:0,chrome:expanded?0:1,angle:0},
    target:expanded?1:0,previous:performance.now(),spinning:false,geometry:null,orientation:null})
  const [motion,setMotion]=useState(timeline.current.motion)
  useLayoutEffect(()=>{
    const state=timeline.current,now=performance.now(),target=expanded?1:0
    sample(state,now)
    if(reduced||!animate){
      state.geometry=null
      state.motion={...state.motion,progress:target,chrome:1-target}
    }else if(target!==state.target){
      state.geometry={at:now,progress:state.motion.progress,chrome:state.motion.chrome,target}
    }
    state.target=target
    const holdOrientation=expanded&&spin&&!reduced
    if(holdOrientation)state.orientation=null
    else if(reduced||!animate){state.orientation=null;state.motion={...state.motion,angle:0}}
    else if(!state.orientation&&state.motion.angle!==0){state.orientation={at:now,angle:upright(state.motion.angle)}}
