...
const fixture = (options = {}) => {
  const sources = {
    roon: new Source(),
    spotify: new Source(),
    appleMusic: new Source(),
    system: new Source(),
  }

  return { sources, music: new NowPlayingSource({ sources, ...options }) }
}
...
test('player settings validate and frames stay below the 2048 byte serial limit', () => {
  const settings = mergeSettings(defaultSettings(), {
    modules: { roon: { enabled: true, players: { spotify: true, system: true } } },
  ...
  assert.throws(
    () =>
      mergeSettings(settings, {
        modules: { roon: { players: { roon: false, spotify: false, system: false } } },
      }),
    /at least one/,
  )
...
test('local Spotify uses bounded commands, maps tracks, and never offers unsupported likes', async (t) => {
  ...
  const source = new LocalMusicSource({
    ...
    runner: async (...args) => {
      ...
      return {
        code: 0,
        stdout: JSON.stringify({ running: true, title: 'Song', artist: 'Artist', playing: true }),
      }
    },
  ...
  source.configure({ enabled: true })
  source.started = true
  await source.poll()

  assert.equal(source.snapshot().track, 'Song')
  assert.equal(source.snapshot().canLike, false)
  await assert.rejects(source.control('like'), /not supported/)

  await source.control('next')

  assert.ok(calls.some((call) => call[1].at(-1) === 'next'))
  assert.equal(calls.at(-1)[2].timeoutMs, 5000)
})
...
test('disabled sources ignore late reads and abort in-flight commands', async () => {
  ...
  source.stop()
  resolveRead({ code: 0, stdout: JSON.stringify({ title: 'Stale', running: true }) })
  await pending
...
test('system controls reject an active player change rather than target another app', async (t) => {
  ...
  const source = new LocalMusicSource({
    ...
    runner: async (command, args) => {
      ...
      return { code: 0, stdout: JSON.stringify({ title: 'Other', bundleIdentifier: 'other.app' }) }
    },
  })
  t.after(() => source.stop())
  source.started = true
  source.enabled = true
  source.bundleIdentifier = 'original.app'
  source.value = { ...source.value, status: 'ready', canNext: true }

  await assert.rejects(source.control('next'), /active Mac player changed/)
  assert.equal(calls.length, 1)
  assert.equal(calls[0].at(-1), 'get')
})
...
test('late artwork conversion cannot replace a newer generation', async (t) => {
  ...
  const source = new LocalMusicSource({
    ...
    runner: async () => ({
      code: 0,
      stdout: JSON.stringify({ title, artworkData: Buffer.from(title).toString('base64') }),
    }),
  ...
  source.enabled = true
  await source.poll()
  resolveOld({ id: 'a'.repeat(40) })
  await old

  assert.equal(source.snapshot().track, 'New')
  assert.equal(source.snapshot().artId, 'b'.repeat(40))
  assert.equal(source.art.id, 'b'.repeat(40))
})
...
test('missing macOS artwork does not erase the current cover and failures can retry', async (t) => {
  ...
  const source = new LocalMusicSource({
    ...
    runner: async () => ({
      code: 0,
      stdout: JSON.stringify({ title: 'Song', ...(hasArt ? { artworkData: 'YQ==' } : {}) }),
    }),

    converter: async () => {
      attempts++

      if (attempts === 1) throw Error('temporary decode failure')

      return { id: 'b'.repeat(40) }
    },
  })
  t.after(() => source.stop())
  source.enabled = true
...
test('Apple Music defaults off, supports independent selection, and survives legacy settings', async () => {
  const settings = mergeSettings(defaultSettings(), {
    modules: { roon: { players: { roon: true, spotify: true, system: true } } },
  })
  ...
  music.configure({
    enabled: true,
    players: { roon: true, spotify: true, appleMusic: true, system: true },
  })
...
test('Apple Music reads local artwork and routes controls to Music, independently of System', async (t) => {
  ...
  const source = new LocalMusicSource({
    ...
    converter: async (bytes, player) => {
      assert.equal(bytes.toString(), 'cover')
      assert.equal(player, 'appleMusic')

      return { id: 'c'.repeat(40) }
    },
  })
  t.after(() => source.stop())
  ...
  assert.equal(source.snapshot().status, 'ready')
  assert.equal(source.snapshot().playing, false)
  assert.equal(source.snapshot().artId, 'c'.repeat(40))
  ...
  assert.ok(
    calls.every(
      (c) =>
        c.command === '/usr/bin/osascript' &&
        c.args[2].endsWith('/apple-music.jxa') &&
        c.options.timeoutMs === 5000,
    ),
  )
  await assert.rejects(source.control('like'), /not supported/)
})
...
test('Apple Music reports closed app and automation denial clearly', async (t) => {
  ...
  const source = new LocalMusicSource({
    ...
    runner: async () => ({ code, stdout: '{"running":false}' }),
  })
  t.after(() => source.stop())
  ...
  assert.match(source.snapshot().error, /Open Music/)
  ...
  assert.match(source.snapshot().error, /control Music/)
})
...
test('track transitions retain the previous frame until metadata and artwork are ready', async (t) => {
  ...
  const source = new LocalMusicSource({
    ...
    runner: async () => ({ code: 0, stdout: JSON.stringify(data) }),
  ...
  data = { running: true, title: 'New', artworkData: 'Yg==' }
  const pending = source.poll()
  ...
  data = { running: true }
  source.missingSince = Date.now() - 9000
  await source.poll()
  clearTimeout(source.timer)
...
test('system controls reject a new app until its artwork and track are displayed', async (t) => {
  ...
  clearTimeout(source.timer)
  let finish, entered
  const converting = new Promise((resolve) => {
    entered = resolve
  })
  source.converter = () =>
    new Promise((resolve) => {
