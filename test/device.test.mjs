...
test('no writes until the expected board announces this protocol', () => {
  ...
  link.receive(ready.slice(0, 15))

  assert.equal(frames.length, 0)

  link.receive(ready.slice(15))
...
test('acknowledgement does not create a serial feedback loop', () => {
  ...
  assert.ok(store.device.lastAck)
...
test('a state change during a write is sent immediately after it', () => {
  const { store, link, frames } = setup()
  let complete
  link.port.write = (frame, callback) => {
    frames.push(JSON.parse(frame))
    complete = callback
  }
  ...
  assert.equal(frames[1].name, 'pi')
  assert.equal(frames[1].seq, store.seq)
...
test('playground selections reach serial and physical cycling returns to live', () => {
  ...
  assert.equal(frames.at(-1).state, 'sleep')
  assert.equal(frames.at(-1).preview, true)
...
test('render diagnostics publish once per ack and reject invalid shimmer counts', () => {
  ...
  assert.equal(store.device.renderedTextGap, 8)
  assert.equal(store.device.renderedStatusTop, 360)

  link.receive(
    JSON.stringify({ ...ack, shimmer_pixels: 25201, text_gap: 16, status_top: 360 }) + '\n',
  )
...
test('obsolete clip diagnostics are ignored without a serial write loop', () => {
  ...
  for (const key of [
    ...
    assert.equal(key in store.device, false)
...
test('device swipes validate direction, obey the setting and persist before changing modules', async () => {
  ...
  const directions = []
  let apply
  link.onModule = (direction) => {
    directions.push(direction)

    return new Promise((resolve) => {
      apply = () => {
        store.cycleModule(direction)
        resolve()
      }
    })
  }
  ...
  for (const direction of [0, 2, '1', null])
    link.receive(JSON.stringify({ type: 'module', v: 1, direction }) + '\n')
...
test('failed module persistence keeps the device on its current module with a safe error', async () => {
  ...
  assert.equal(store.activeModule, 'face')
  assert.match(store.device.error, /Could not save/)
  assert.doesNotMatch(store.device.error, /private/)
})
...
test('vertical usage gestures validate protocol and state independently of module swipes', async () => {
  ...
  const page = (direction) =>
    link.receive(JSON.stringify({ type: 'usage-page', v: 1, direction }) + '\n')
  ...
  assert.equal(frames.at(-1).dashboard.pageIndex, 1)
  ...
  store.setModule('usage')
  store.setSources({ usage: { ...sources.usage, status: 'loading' } })
  page(1)
  await turn()
  ...
  store.setSources(sources)
  store.setSettings(mergeSettings(store.settings, { modules: { usage: { enabled: false } } }))
  page(1)
  await turn()
...
test('vertical mailbox gestures validate protocol and require the current ready HEY module', async () => {
  ...
  const page = (direction) =>
    link.receive(JSON.stringify({ type: 'hey-page', v: 1, direction }) + '\n')
  ...
  assert.equal(frames.at(-1).dashboard.items[0].sender, 'Sender 2')
  ...
  store.setModule('hey')
  store.setSources({ hey: { ...sources.hey, status: 'loading' } })
  page(1)
  await turn()
  ...
  store.setSources(sources)
  store.setSettings(mergeSettings(store.settings, { modules: { hey: { enabled: false } } }))
  page(1)
  await turn()
...
test('panel transfer diagnostics distinguish accepted settings from pixel transfers', () => {
  ...
  link.receive(
    JSON.stringify({ ...ack, panel_transfers: 9, panel_error: 0, panel_rotation: 22 }) + '\n',
  )
...
test('panel failures remain visible on healthy USB and clear only their own recovered error', () => {
  ...
  assert.equal(store.device.status, 'connected')
  assert.equal(store.device.panelError, 257)
  assert.match(store.device.error, /Display transfer failed \(257\)/)
  assert.match(store.device.error, /USB is connected/)

  const failure = store.device.error
  receive({ panel_error: -1 })

  assert.equal(store.device.error, failure)

  receive({ panel_error: 0 })

  assert.equal(store.device.error, null)
  assert.equal(store.device.panelError, 0)

  receive({ panel_error: 258 })

  assert.match(store.device.error, /258/)

  store.setDevice({ error: 'Another error' })
  receive({ panel_error: 0 })

  assert.equal(store.device.error, 'Another error')
...
test('ready metadata publishes the registered profile and rejects conflicting screens', () => {
  ...
  link.receive(
    JSON.stringify({
      type: 'ready',
      v: 1,
      board: 'waveshare-1.75-b',
      display: { width: 480, height: 480, shape: 'round' },
    }) + '\n',
  )
  ...
  link.receive(
    JSON.stringify({
      ...
      v: 1,
      board: 'waveshare-1.75-b',
      display: { width: 466, height: 466, shape: 'round' },
    }) + '\n',
  )
...
test('Audio serial controls require enabled active module and valid absolute targets', async () => {
  const { store, link } = setup()
  store.settings.modules.audio = { enabled: true }
  store.activeModule = 'audio'
  link.ready = true
  const controls = [],
    pages = []
  link.onAudioControl = (request) => controls.push(request)
  link.onAudioPage = (direction) => pages.push(direction)
  ...
  send(control)
  send({ ...control, scope: 'input', action: 'mute', value: true })
  send({ ...control, action: 'device', value: 42 })
  send({ type: 'audio-page', direction: -1 })
  ...
  for (const fields of [
    ...
    { value: '50' },
    { action: 'mute', value: 1 },
    { action: 'device', value: 0 },
    { action: 'device', value: 1.5 },
    { action: 'bad' },
  ])
    send({ ...control, ...fields })
  ...
  assert.deepEqual(controls, [
    { scope: 'output', deviceId: 17, action: 'volume', value: 45 },
    { scope: 'input', deviceId: 17, action: 'mute', value: true },
    { scope: 'output', deviceId: 17, action: 'device', value: 42 },
  ])
  ...
  send({ type: 'audio-page', direction: 1 })
  store.activeModule = 'audio'
  store.settings.modules.audio.enabled = false
  send(control)
  await turn()
...
test('Audio picker view validates current module and device identity shape', async () => {
  ...
  link.ready = true
  const calls = []
  link.onAudioView = (request) => calls.push(request)
  ...
  assert.deepEqual(calls, [{ open: true, scope: 'input', deviceId: 17 }])
