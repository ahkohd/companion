...
const agent = (id, state, extra = {}) => ({ pane_id: id, agent: 'pi', name: id, agent_status: state, ...extra });
...
test('selection cycles and resets when its agent disappears', () => {
  const store = new FaceStore(); store.ingest([agent('a', 'idle'), agent('b', 'working')]);
  store.cycle(); assert.equal(store.selected, 'b');
  store.cycle(); assert.equal(store.selected, 'a');
  store.cycle(); assert.equal(store.selected, 'all');
  store.select('a'); store.ingest([agent('b', 'working')]);
...
test('missing or disconnected agents cannot report success', () => {
  const store = new FaceStore(); store.ingest([agent('a', 'brand-new-state')]);
  assert.equal(store.display().state, 'unknown');
  store.select('a'); store.disconnect();
  assert.equal(store.frame().state, 'disconnected');
  assert.equal(store.frame().name, '');
  store.ingest([]);
...
test('the wire frame is bounded and excludes raw session data', () => {
  const store = new FaceStore();
  store.ingest([agent('a', 'working', { name: 'x'.repeat(1000), cwd: '/private/project', secret: 'not exported' })]);
  store.select('a');
...
test('device acknowledgements do not change the face revision', () => {
  const store = new FaceStore(); store.ingest([agent('a', 'idle')]);
  const { seq, changedAt } = store;
  store.setDevice({ lastAck: Date.now() });
  assert.equal(store.seq, seq); assert.equal(store.changedAt, changedAt);
});

test('spacing reaches both displays without changing selection or animation age', () => {
  const store = new FaceStore(); store.ingest([agent('a', 'working')]); store.select('a');
  store.changedAt = Date.now() - 5000;
  const { changedAt, animationEpoch } = store;
  ...
  const seq = store.seq;
  store.setTextGap(16); assert.equal(store.seq, seq);
...
test('wire clock and state age keep sleep aligned across updates and reconnection', () => {
  const store=new FaceStore();store.ingest([agent('a','idle')]);store.select('a');
  store.changedAt=Date.now()-40000;
  store.ingest([agent('a','idle',{name:'Updated title'})]);
  store.setPointer({enabled:true,status:'active',x:.5,y:-.5});
  ...
  assert.equal(frame.state,'idle');assert.equal(frame.preview,false);
  assert.ok(store.snapshot().animationMs>=frame.animationMs);
  assert.ok(Math.abs(store.snapshot().ageMs-frame.ageMs)<100);
  store.setExpression('idle');assert.equal(store.frame().preview,true);assert.ok(store.frame().ageMs<100);
});

test('playground overrides the wire state while Herdr keeps updating underneath', () => {
  const store = new FaceStore(); store.ingest([agent('a', 'working')]);
  store.select('a');
  for (const [expression, label] of [['working', 'Working'], ['blocked', 'Needs your input'], ['done', 'Ready'], ['idle', 'Idle'], ['sleep', 'Sleeping']]) {
...
test('playground works without Herdr and selection or touch clears it', () => {
  const store = new FaceStore(); store.setExpression('idle');
  assert.equal(store.frame().state, 'idle'); assert.equal(store.connected, false);
  store.disconnect(); assert.equal(store.frame().state, 'idle');
  store.cycle(); assert.equal(store.expression, null); assert.equal(store.frame().state, 'disconnected');
  store.setExpression('done'); store.select('all'); assert.equal(store.expression, null);
