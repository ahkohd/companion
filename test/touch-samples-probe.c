...
static module_touch_action_t drain(touch_samples_t *queue)
{
    ...
    while (touch_samples_pop(queue, &sample)) {
        if (!sample.valid) { contact.active = false; continue; }
        if (sample.pressed) {
            if (!contact.active) module_touch_begin(&contact, sample.x, sample.y, sample.time_us);
            else module_touch_move(&contact, sample.x, sample.y);
        } else result = module_touch_end(&contact, sample.x, sample.y, sample.time_us);
    }
    return result;
}
int main(void)
{
    ...
    for (int i=0; i<300; ++i) touch_samples_push(&queue, 200, 200, true, true, 49000 + i*8000);
    touch_samples_push(&queue, 0, 0, false, true, 49000 + 300*8000);
    assert(queue.revision == 0); assert(queue.count == 2);
    ...
    for (int i=0; i<TOUCH_SAMPLE_CAPACITY+10; ++i) touch_samples_push(&queue, i, 200, true, true, 1000+i*8000);
    assert(queue.revision == 1); assert(queue.blocked);
    assert(drain(&queue) == MODULE_TOUCH_NONE);
    ...
    assert(queue.blocked); assert(drain(&queue) == MODULE_TOUCH_NONE);
    touch_samples_push(&queue, 0, 0, false, true, 25000);
    ...
    assert(queue.revision == 1); assert(drain(&queue) == MODULE_TOUCH_NONE);
    ...
    assert(queue.revision == 0); assert(drain(&queue) == MODULE_TOUCH_TAP);
    touch_samples_push(&queue, 200, 200, true, true, 901000);
    touch_samples_push(&queue, 0, 0, false, true, 1901000);
    assert(queue.revision == 0); assert(drain(&queue) == MODULE_TOUCH_NONE);
