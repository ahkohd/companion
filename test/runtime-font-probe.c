...
{
    char path[1024];
    snprintf(path, sizeof(path), "%s-%u-%d.ppm", prefix, role, size);
...
int main(int argc, char **argv)
{
    ...
    assert(title->line_height == design_font_line(31));
    assert(!design_fonts_have_error());
    ...
    lv_obj_set_size(label, 416, 220);
    lv_obj_set_pos(label, 25, 100);
    ...
    for (unsigned role = 0; role < DESIGN_FONT_ROLE_COUNT; ++role) {
        ...
        for (int size = pixel_role ? 24 : 12; size <= (pixel_role ? 160 : 48); ++size) {
            ...
            bool bitmap = pixel_role ? size == 44 || size == 56 ||
                                           (size == 128 && role == DESIGN_FONT_CLOCK_TIME)
                                     : size == 16 || size == 22 || size == 28;
            if (!bitmap) {
                if (runtime)
                    assert(runtime == font);
                ...
                assert(
                    font->line_height ==
                    (role == DESIGN_FONT_AUDIO_PICKER && size == 26 ? 30 : design_font_line(size)));
            }

            const char *text =
                pixel_role ? role == DESIGN_FONT_USAGE_VALUE ? "100%." : "12:59." : "Hello, world!";

            for (const char *p = text; *p; ++p) {
                lv_font_glyph_dsc_t glyph;

                assert(lv_font_get_glyph_dsc(font, &glyph, *p, p[1]));
            ...
            lv_label_set_text(label,
                              pixel_role ? text : "Session \xcf\x80 - fixing the playground");
            ...
            if (argc > 1 && ((role == DESIGN_FONT_FACE_NAME && (size == 16 || size == 19)) ||
                             (role == DESIGN_FONT_HEY_SENDER && size == 31) ||
                             (role == DESIGN_FONT_CLOCK_TIME && size == 113)))
                save(argv[1], role, size);
            cases++;
        }
    ...
    for (int repeat = 0; repeat < 50; ++repeat) {
        ...
        assert(a->line_height == design_font_line(31));
        assert(b->line_height == design_font_line(19));
    }
    ...
    printf(
        "PASS: %u role/size combinations rasterized; pi available at every Sans size; stable isolated handles, exact metrics and allocation-failure reporting\n",
        cases);
