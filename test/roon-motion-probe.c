...
static void same(double a, double b)
{
    assert(fabs(a - b) < .000001);
}
...
int main(void)
{
    ...
    p = roon_motion_sample(&motion, 1.140);
    same(p.size, 400);

    assert(p.chrome > 0 && p.chrome < .02);

    p = roon_motion_sample(&motion, 1.180);
    same(p.chrome, 0);

    assert(p.size < 430);

    p = roon_motion_sample(&motion, 1.280);
    same(p.x, 18);
    same(p.y, 18);
    same(p.size, 430);
    same(p.radius, 215);
    ...
    for (int animate = 0; animate <= 1; ++animate)
        for (int spin = 0; spin <= 1; ++spin) {
            ...
            p = roon_motion_sample(&motion, 1);
            same(p.size, animate ? 190 : 430);
            same(p.chrome, animate ? 1 : 0);

            assert(roon_motion_settled(&motion) == !animate);

            p = roon_motion_sample(&motion, 6);
            same(p.size, 430);
            same(p.angle, spin ? 90 : 0);
            ...
            p = roon_motion_sample(&motion, 21.28);
            same(p.size, 190);
            same(fmod(p.angle, 360), 0);
    ...
    p = roon_motion_sample(&motion, 12.14);

    assert(p.size > 190);

    roon_motion_set(&motion, false, false, false, true, 138, 70, 190, 16, 12.14);
    same(motion.pose.size, 190);
    same(motion.pose.chrome, 1);
    ...
    printf(
        "PASS: all4 motion settings, instant default,280ms geometry,180ms chrome,20s spin, pause/resume, opt-out upright, custom restore, clock reset and %u interruption cases\n",
        cases);
}
