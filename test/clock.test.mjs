...
test('the clock formats local time with twelve-hour defaults, noon, midnight and weekday rollover', () => {
  ...
  for (const [hour, minute, time] of [
    ...
    assert.equal(dashboard.time, time)
    assert.equal(dashboard.weekday, '')
  }

  const disabled = dashboardFor(
    'clock',
    {},
  ...
  assert.equal(disabled.time, undefined)
})
...
test('saved three-module settings acquire clock defaults without losing existing preferences', async (t) => {
  ...
  const saved = mergeSettings(defaultSettings(), {
    ...
    device: {
      activeModule: 'hey',
      moduleOrder: ['hey', 'face', 'usage', 'clock', 'roon', 'audio', 'speedDial'],
      textGap: 4,
      followMouse: true,
      mouseInterval: 250,
      swipeEnabled: false,
      showModuleNavigation: true,
      showCardBackgrounds: true,
    },
    modules: {
      usage: { enabled: true, providers: ['codex'], provider: 'codex', refreshSeconds: 120 },
      hey: { enabled: true, box: 'feed', refreshSeconds: 300 },
    },
    mappings: { working: 'done', disconnected: 'sleep' },
  })
  const expected = structuredClone(saved)
  delete saved.modules.clock
  delete saved.modules.roon
  delete saved.design.roon
  saved.device.moduleOrder = ['hey', 'face', 'usage']
  ...
  await settings.save({
    modules: {
      clock: { enabled: true, hourFormat: '24', showWeekday: false, blinkSeparator: false },
    },
  })
  ...
  assert.deepEqual(restarted.value.modules.clock, {
    enabled: true,
    hourFormat: '24',
    showWeekday: false,
...
test('all four modules cycle in configured order and disabling clock chooses the next enabled module', () => {
  ...
  assert.deepEqual(
    ...
    MODULE_IDS.filter((id) => !['roon', 'audio', 'speedDial'].includes(id)),
  )
  ...
  assert.deepEqual(store.enabledModules(), ['face', 'usage', 'hey'])
})
...
test('minute changes publish the clock to browser and device without restarting animation or changing gaze', (t) => {
  ...
  store.setSettings(clockSettings())
  ...
  assert.equal(initial.display.dashboard.status, 'ready')
  ...
  assert.equal(frames.at(-1).module, 'clock')
  assert.equal(frames.at(-1).dashboard.time, '12:00')
  assert.equal(store.changedAt, initial.changedAt)
  assert.equal(frames.at(-1).epoch, frames[0].epoch)
...
test('clock frames stay bounded and clock preferences do not launch a source collector', async (t) => {
  ...
  assert.equal(frame.dashboard.time, '12:59')
  assert.ok(Buffer.byteLength(frame.dashboard.time) <= 7)
  ...
  const source = new ModuleSources({
    settings: store.settings.modules,

    runner: async (command, args) => {
      calls.push([command, args])

      return { code: 0, stdout: 'CLI 1.0.0' }
    },
  })
  t.after(() => source.stop())
  await source.start()
  ...
  await assert.rejects(source.refresh('clock'), /usage or hey/)
})
...
test('clock blinking can be disabled without changing the time text', () => {
  ...
  assert.equal(off.time, on.time)
  assert.equal(on.blinkSeparator, true)
})
