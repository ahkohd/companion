...
const artwork = (id) => ({
  ...
  width: 160,
  height: 160,
  pixels: Buffer.from(Array.from({ length: 51200 }, (_, i) => i % 251)),
})
...
before(async () => {
  ...
  execFileSync('cc', [
    ...
    '-I',
    cjson,
    'test/roon-art-probe.c',
    'firmware/main/roon_artwork.c',
...
function ack(link, patch = {}) {
  ...
  link.receive(
    JSON.stringify({
      type: 'artAck',
      v: 1,
      transfer: packet.frame.transfer,
      op: packet.frame.op,
      ok: true,
      offset: packet.expected,
      ...patch,
    }) + '\n',
  )
}
...
function finish(link) {
  for (let i = 0; link.artPacket && i < 100; i++) ack(link)

  assert.equal(link.artPacket, null)
}
...
test('artwork upload is bounded, acknowledged, copied from the caller and reuploaded after ready', (t) => {
  const { link, frames } = setup(t),
    art = artwork('a'),
    expected = Buffer.from(art.pixels)
  ...
  link.receive(ready)
  const initial = frames.filter((frame) => frame.type === 'art')

  assert.equal(initial.length, 1)
  assert.equal(initial[0].op, 'begin')

  ack(link, { transfer: initial[0].transfer + 1 })

  assert.equal(frames.filter((frame) => frame.type === 'art').length, 1)

  ack(link, { offset: 51201 })
  ...
  assert.notEqual(link.artPacket.frame.transfer, initial[0].transfer)
...
test('state writes retain priority during artwork writes and while waiting for artwork acknowledgements', (t) => {
  ...
  link.receive(ready)
  let complete
  link.port.write = (frame, done) => {
    frames.push(JSON.parse(frame))
    complete = done
  }
  link.setArtwork(artwork('a'))

  assert.equal(frames.at(-1).type, 'art')

  store.setExpression('working')

  assert.equal(frames.at(-1).type, 'art')

  complete()

  assert.equal(frames.at(-1).type, 'state')
  assert.equal(frames.at(-1).seq, store.seq)
  ...
  assert.equal(frames.at(-1).op, 'chunk')
...
test('changing tracks cancels stale artwork acknowledgements and clearing art sends a bounded cancel', (t) => {
  ...
  link.receive(ready)
  link.setArtwork(artwork('a'))
  const stale = { ...link.artPacket.frame }
  link.setArtwork(artwork('b'))
  const transfer = link.artPacket.frame.transfer
  link.receive(JSON.stringify({ ...stale, type: 'artAck', ok: true, offset: 0 }) + '\n')

  assert.equal(link.artPacket.frame.transfer, transfer)
  assert.equal(link.artPacket.frame.op, 'begin')

  finish(link)
  link.setArtwork(null)

  assert.equal(frames.at(-1).op, 'cancel')

  ack(link)

  assert.equal(link.artTransfer, null)
  assert.equal(link.artwork, null)
})
...
test('artwork rejects invalid buffers, retries a dropped packet and reports a failed upload without blocking state', (t) => {
  ...
  link.receive(ready)
  link.setArtwork(artwork('a'))
  const packet = frames.at(-1)
  t.mock.timers.tick(1500)

  assert.deepEqual(frames.at(-1), packet)
  ...
  assert.match(store.device.error, /did not acknowledge/)
  assert.equal(link.artTransfer, null)
...
test('native artwork receiver matches bridge uploads and keeps committed pixels atomic through retries', {
  ...
  assert.equal(results.at(-1).id, 'a'.repeat(40))
  assert.equal(results.at(-1).crc32, upload[0].crc32)
  assert.equal(results.at(-1).revision, 1)
})

test('native artwork receiver rejects malformed chunks, incomplete images, stale transfers and bad checksums', {
  skip: !nativeAvailable,
}, (t) => {
  const { link, frames } = setup(t)
  ...
  const invalid = [
    { ...begin, width: 159 },
    { ...begin, id: 'g'.repeat(40) },
    { ...chunk, offset: 1 },
    { ...chunk, offset: -1 },
    { ...chunk, offset: 51200 },
    ...['!', 'AAAAA', 'AB==', 'AAB=', '=AAA', 'AA=A', 'AAAA'.repeat(257)].map((data) => ({
      ...chunk,
      data,
    })),
    commit,
  ...
  assert.equal(invalidResults[0].ok, true)
  assert.ok(invalidResults.slice(1).every((result) => !result.ok))
  ...
  assert.equal(badChecksum.at(-1).ok, false)
  assert.equal(badChecksum.at(-1).id, '')

  const cleared = native([...upload, { ...commit, op: 'cancel' }, chunk])

  assert.equal(cleared.at(-2).ok, true)
  assert.equal(cleared.at(-2).id, '')
  assert.equal(cleared.at(-1).ok, false)
})

test('bridge restart can replace committed art or resume a partial image even with a reused transfer number', {
  skip: !nativeAvailable,
}, (t) => {
  const first = setup(t)
  first.link.artCounter = 0

  first.link.receive(ready)
  first.link.setArtwork(artwork('a'))
  finish(first.link)
  const original = first.frames.filter((frame) => frame.type === 'art')
  const replacement = setup(t)
  replacement.link.artCounter = 0
  const changed = artwork('b')
  changed.pixels.fill(0x3c)
  replacement.link.receive(ready)
  replacement.link.setArtwork(changed)
  finish(replacement.link)
  ...
  assert.equal(complete.at(-1).id, changed.id)
  assert.equal(complete.at(-1).crc32, next[0].crc32)
  assert.equal(complete.at(-1).revision, 2)

  const resumed = setup(t)
  resumed.link.artCounter = 0
  resumed.link.receive(ready)
  resumed.link.setArtwork(artwork('a'))
  ack(resumed.link, { offset: 768 })

  assert.equal(resumed.link.artPacket.frame.offset, 768)
  ...
  assert.equal(partial[2].offset, 768)
  assert.equal(partial.at(-1).id, 'a'.repeat(40))
...
test('device Roon controls require an enabled active module and validated actions', async (t) => {
  const { link, store } = setup(t),
    actions = []
  link.onRoonControl = (action) => actions.push(action)

  const control = (action) =>
    link.receive(JSON.stringify({ type: 'roon-control', v: 1, action }) + '\n')
  ...
  assert.deepEqual(actions, ['previous', 'next', 'playpause'])
...
test('device app badges require an explicit supported player and never send playback or view actions', async (t) => {
  ...
  const open = (player) =>
    link.receive(JSON.stringify({ type: 'roon-control', v: 1, action: 'open', player }) + '\n')
  ...
  assert.deepEqual(
    actions,
    ['roon', 'spotify', 'appleMusic'].map((player) => ({ action: 'open', player })),
  )

  store.attention = { active: { id: 'overlay' } }
  open('roon')
  await turn()

  assert.equal(actions.length, 3)

  store.attention = null
  store.setModule('face')
  open('roon')
  await turn()
