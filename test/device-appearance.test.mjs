...
test('system changes update every module wire palette without changing saved preference', () => {
  ...
  const seq = store.seq;
  store.setSystemAppearance('dark'); assert.equal(store.seq, seq);
});
...
test('palette validation rejects invalid RGB and keeps inactive palette separate', () => {
  ...
  for (const background of [-1,0x1000000,1.5,'#ffffff',null])
    assert.throws(()=>mergeSettings(settings,{deviceAppearance:{palettes:{light:{background}}}}));
  const changed=mergeSettings(settings,{deviceAppearance:{palettes:{light:{background:0xfafafa}}}});
...
test('host appearance reads OS preference and retains state on query failures', async () => {
  assert.equal(await readSystemAppearance({platform:'darwin',run:async()=>({stdout:'Dark\n'})}),'dark');
  assert.equal(await readSystemAppearance({platform:'darwin',run:async()=>{throw {code:1,stderr:'The domain/default pair does not exist'};}}),'light');
  assert.equal(await readSystemAppearance({platform:'darwin',run:async()=>{throw {code:'ETIMEDOUT'};}}),null);
  assert.equal(await readSystemAppearance({platform:'win32',run:async()=>({stdout:'AppsUseLightTheme    REG_DWORD    0x0'})}),'dark');
  assert.equal(await readSystemAppearance({platform:'linux',run:async()=>({stdout:"'prefer-light'"})}),'light');
});

test('stopping an in-flight host appearance read prevents late updates', async () => {
  let finish; const seen=[];
  const monitor=new SystemAppearance(v=>seen.push(v),{read:()=>new Promise(r=>finish=r)});
  const started=monitor.start(); monitor.stop(); finish('light'); await started;
  assert.deepEqual(seen,[]); assert.equal(monitor.timer,undefined);
});
