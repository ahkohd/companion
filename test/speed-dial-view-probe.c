...
static void save(const char *path, bool rectangular)
{
    ...
    for (unsigned i = 0; i < 466 * 466; ++i) {
        ...
        int x = i % 466 - 233, y = i / 466 - 233;
        unsigned char rgb[3] = {((p >> 11) & 31) * 255 / 31, ((p >> 5) & 63) * 255 / 63,
                                (p & 31) * 255 / 31};
        if (!rectangular && x * x + y * y > 233 * 233)
            memset(rgb, 24, sizeof(rgb));
        fwrite(rgb, 1, 3, file);
    }
...
int main(void)
{
    ...
    module_snapshot_t module = {.kind = DISPLAY_SPEED_DIAL,
                                .count = 7,
                                .show_navigation = true,
                                .status = MODULE_READY,
                                .page_count = 2,
                                .speed_dial = {.grid_size = 0,
                                               .list_rows = 4,
                                               .show_labels = true,
                                               .rectangular = true,
                                               .count = 9},
                                .palette = {0, 0xf2edfa, 0x958ca4, 0x151515, 0x2b2b2b, 0x65c18c,
                                            0x65c18c, 0xd9be81, 0xe88483}};
    memset(module.open_token, 'a', 40);
    memset(module.art_id, 'b', 40);
    ...
    for (unsigned i = 0; i < SPEED_DIAL_BUTTON_LIMIT; ++i) {
        ...
        snprintf(module.speed_dial.buttons[i].id, sizeof(module.speed_dial.buttons[i].id),
                 "button-%u", i);
        snprintf(module.speed_dial.buttons[i].label, sizeof(module.speed_dial.buttons[i].label),
                 "Button %u", i + 1);
    ...
    for (unsigned labels = 0; labels < 2; ++labels) {
        ...
        lv_refr_now(display);
        speed_dial_geometry_t geometry;

        assert(speed_dial_geometry(&module, 0, &geometry));

        speed_dial_target_t unavailable = {0};
        int x = geometry.button.x + geometry.button.width / 2,
            y = geometry.button.y + geometry.button.height / 2;

        assert(!module_view_speed_dial_press(x, y, &unavailable));
        assert(!module_view_speed_dial_press_valid(x, y, &unavailable));

        for (unsigned i = 0; i < SPEED_DIAL_BUTTON_LIMIT; ++i)
            assert(!lv_obj_is_visible(speed_dial_images[i]));
    }

    module_view_set_artwork("cccccccccccccccccccccccccccccccccccccccc", (uint8_t *)atlas);
    module_view_update(&module, false);
    speed_dial_geometry_t unavailable_geometry;

    assert(speed_dial_geometry(&module, 0, &unavailable_geometry));

    speed_dial_target_t unavailable = {0};

    assert(!module_view_speed_dial_press(
        unavailable_geometry.button.x + unavailable_geometry.button.width / 2,
    ...
    for (unsigned i = 0; i < module.speed_dial.count; ++i) {
        ...
        for (unsigned y = 8; y < 40; ++y)
            for (unsigned x = 8; x < 40; ++x)
                assert(screen_pixels[(g.icon.y + y) * 466 + g.icon.x + x] ==
                       (uint16_t)((i + 1) * 0x0765));
    }
    ...
    lv_area_t footer_bounds;
    lv_obj_get_coords(speed_dial_footer, &footer_bounds);

    for (unsigned i = 0; i < 7; ++i) {
        lv_area_t dot_bounds;
        lv_obj_get_coords(dots[i], &dot_bounds);
    ...
    lv_text_get_size(&page_size, "1 / 2", &lv_font_geist_mono_16, 0, 0, LV_COORD_MAX,
                     LV_TEXT_FLAG_NONE);
    ...
    for (unsigned count = 1; count <= SPEED_DIAL_BUTTON_LIMIT; ++count) {
        ...
        if (count == 9)
            save(".work/speed-dial-nine-native.ppm", false);
    }
    ...
    speed_dial_geometry_t centred;

    assert(speed_dial_geometry(&module, 0, &centred));
    ...
    assert(speed_dial_geometry(&module, 0, &g));
    ...
    module.art_id[0] = 'b';
    module.speed_dial.list = true;
    display_module_design(&module, &module.design);
    module.has_design = true;
    module.design.speedDial.labelSize = 26;

    for (unsigned i = 0; i < 4; ++i) {
        module.speed_dial.buttons[i].status = i;
        module.speed_dial.buttons[i].has_color = true;
        module.speed_dial.buttons[i].color = 0xff0000;
    }

    for (unsigned light = 0; light < 2; ++light)
        for (unsigned rows = 3; rows <= 4; ++rows) {
            ...
            for (unsigned i = 0; i < rows; ++i) {
                ...
                assert(lv_obj_get_style_text_font(speed_dial_labels[i], 0) ==
                       design_font(DESIGN_FONT_SPEED_DIAL_LABEL, 26));
                ...
                if (hittable)
                    assert(!strcmp(list_target.id, module.speed_dial.buttons[i].id));
            }

            save(light ? ".work/speed-dial-list-light-native.ppm"
                       : ".work/speed-dial-list-native.ppm",
                 false);
        }

    module.speed_dial.list = false;
    module.speed_dial.count = speed_dial_layout(&module, NULL);
    module_view_update(&module, false);

    for (unsigned i = 0; i < module.speed_dial.count; ++i)
        assert(lv_obj_get_style_bg_opa(speed_dial_panels[i], 0) == LV_OPA_COVER);

    module.speed_dial.grid_size = 5;
    module_view_update(&module, false);

    for (unsigned i = 0; i < SPEED_DIAL_BUTTON_LIMIT; ++i)
        assert(!lv_obj_is_visible(speed_dial_panels[i]));
    assert(!module_view_speed_dial_press(x, y, &target));
    assert(!design_fonts_have_error());

    puts(
        "Native LVGL atlas stride, pixels, identity, pressed feedback and stale-touch checks passed");
}
