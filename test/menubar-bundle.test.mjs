...
test('production bundle preserves dependency cycles with internal links and excludes development packages', async () => {
  ...
  const root = path.join(temporary, 'source'),
    destination = path.join(temporary, 'bundle/node_modules')
  ...
  try {
    ...
    const a = await realpath(path.join(destination, 'a')),
      b = await realpath(path.join(a, 'node_modules/b'))
    const actualDestination = await realpath(destination)

    assert.ok(a.startsWith(actualDestination))
    assert.ok(b.startsWith(actualDestination))
    assert.equal(await realpath(path.join(b, 'node_modules/a')), a)
    assert.equal(JSON.parse(await readFile(path.join(b, 'package.json'))).name, 'b')
    assert.ok(!(await readdir(destination)).includes('development-only'))

    await rm(root, { recursive: true })

    assert.equal(
      JSON.parse(await readFile(path.join(destination, 'a/node_modules/b/package.json'))).name,
      'b',
    )
  } finally {
