...
size_t heap_caps_get_free_size(unsigned caps) { (void)caps; return 1000000; }
size_t lv_display_get_draw_buf_size(lv_display_t *display) { return display->bytes; }
bool lv_display_flush_is_last(lv_display_t *display) { return display->last; }
esp_err_t esp_lv_adapter_set_draw_bitmap_callbacks(lv_display_t *display,
    const esp_lv_adapter_draw_bitmap_callbacks_t *callbacks, void *context)
{
    (void)display; (void)context; registered = *callbacks; return ESP_OK;
}
esp_err_t esp_lcd_panel_draw_bitmap(esp_lcd_panel_handle_t panel, int x1, int y1, int x2, int y2, const void *data)
{
    (void)panel; assert(!in_flight); draws++; in_flight = data;
...
static esp_err_t flush(lv_display_t *display, screen_rect_t area, bool last)
{
    ...
    display->last = last;
    return registered.custom_draw_bitmap(display, NULL, area.x, area.y,
        area.x + area.width, area.y + area.height, pixels, NULL);
}
static void reset(void)
{
    in_flight = NULL;
    heap_caps_free(scratch); heap_caps_free(pending_scratch); heap_caps_free(backing);
    scratch = pending_scratch = backing = NULL; scratch_pixels = pending_pixels = 0;
    current_rotation = 0; force_full = false; failed_rotation = UINT_MAX; buffered_dirty = (screen_rect_t){0};
...
int main(void)
{
    ...
    for (int y = 0; y < SCREEN_SIDE; y += 50) {
        ...
        assert(flush(&display, (screen_rect_t){0, y, SCREEN_SIDE, height}, y + height == SCREEN_SIDE) == ESP_OK);
    }
    ...
    assert(draws == before + 1 && last_area.width == SCREEN_SIDE && last_area.height == SCREEN_SIDE);
    screen_rect_t expected_area;
    assert(screen_rotate_backing(&transform, backing, (screen_rect_t){0, 0, SCREEN_SIDE, SCREEN_SIDE}, expected, SCREEN_SIDE * SCREEN_SIDE, &expected_area));
    ...
    for (int y = 0; y < SCREEN_SIDE; y += 50) {
        ...
        assert(flush(&display, (screen_rect_t){0, y, SCREEN_SIDE, height}, last) == (last ? ESP_OK : ESP_ERR_NOT_ALLOWED));
    }
    assert(draws == before + 1 && last_area.width == SCREEN_SIDE && last_area.height == SCREEN_SIDE);
    ...
    before = draws;
    assert(flush(&display, (screen_rect_t){464, 0, 2, 2}, true) == ESP_ERR_NOT_ALLOWED && draws == before);
    reset();
    ...
    assert(flush(&display, (screen_rect_t){20, 20, 2, 2}, true) == ESP_OK);
    reset();
    puts("PASS: coalesced partial/full flushes, offscreen completion, deferred DMA buffer replacement and graceful allocation failures");
}
