...
static bool label_hits_circle(speed_dial_rect_t label, speed_dial_rect_t button)
{
    ...
    double x = fmax(label.x, fmin(cx, label.x + label.width)),
           y = fmax(label.y, fmin(cy, label.y + label.height));
...
{
    if (module->speed_dial.list || module->speed_dial.grid_size)
        return;
    ...
    for (unsigned i = 0; i < count; ++i) {
        if (module->page_count == 1 && i >= module->speed_dial.count)
            continue;
        ...
        assert(button.x >= 12 && button.x + button.width <= 454 && button.y >= 12 &&
               button.y + button.height <= (dense ? 454 : 408));
        ...
        if (dense) {
            ...
            if (i % 2) {
                ...
                assert(2 * button.x + button.width + 2 * opposite.x + opposite.width ==
                       2 * (2 * middle.x + middle.width));
                assert(2 * button.y + button.height + 2 * opposite.y + opposite.height ==
                       2 * (2 * middle.y + middle.height));
            }

            if (module->page_count > 1)
                assert(speed_dial_clears_controls(button.x, button.y, button.width));
        ...
        if (!module->speed_dial.rectangular) {
            double dx = button.x + button.width / 2.0 - 233,
                   dy = button.y + button.height / 2.0 - 233;
            ...
            if (!module->speed_dial.rectangular)
                for (unsigned corner = 0; corner < 4; ++corner) {
                    int x = label.x + (corner % 2 ? label.width : 0) - 233,
                        y = label.y + (corner / 2 ? label.height : 0) - 233;
        ...
        for (unsigned j = i + 1; j < count; ++j) {
            ...
            double dy = button.y + button.height / 2.0 - other.y - other.height / 2.0;
            double spacing =
                (button.width + other.width) / 2.0 + (dense ? design.speedDial.gap : 0);

            assert(dx * dx + dy * dy >= spacing * spacing);

            if (label.width && slots[j].label.width) {
                assert(!rectangles_overlap(label, slots[j].label));
                assert(!label_hits_circle(label, other) &&
                       !label_hits_circle(slots[j].label, button));
            }
        }
    }
}
...
static void check_targets(const module_snapshot_t *module, unsigned rotation_step)
{
    ...
    for (unsigned i = 0; i < module->speed_dial.count; ++i) {
        ...
        assert(slot.button.x >= 0 && slot.button.y >= 0 &&
               slot.button.x + slot.button.width < 466 && slot.button.y + slot.button.height < 466);

        int x = slot.button.x + slot.button.width / 2, y = slot.button.y + slot.button.height / 2;
        bool enabled = module->speed_dial.buttons[i].enabled &&
                       module->speed_dial.buttons[i].status != SPEED_DIAL_RUNNING;
        ...
        for (unsigned rotation = 0; rotation < 360; rotation += rotation_step) {
            ...
            int physical_x =
                lround(232.5 + (x - 232.5) * cos(radians) - (y - 232.5) * sin(radians));
            int physical_y =
                lround(232.5 + (x - 232.5) * sin(radians) + (y - 232.5) * cos(radians));
        ...
        changed = *module;
        changed.speed_dial.buttons[i].enabled = false;

        assert(!speed_dial_target_valid(&changed, x, y, &target));

        changed = *module;
        changed.speed_dial.buttons[i].status = SPEED_DIAL_RUNNING;

        assert(!speed_dial_target_valid(&changed, x, y, &target));

        changed = *module;
        changed.speed_dial.buttons[i].id[0] = '_';

        assert(!speed_dial_target_valid(&changed, x, y, &target));

        changed = *module;
        changed.speed_dial.show_labels = !changed.speed_dial.show_labels;

        assert(!speed_dial_target_valid(&changed, x, y, &target));

        changed = *module;
        display_module_design(module, &changed.design);
        changed.has_design = true;
        changed.design.speedDial.gap++;

        assert(!speed_dial_target_valid(&changed, x, y, &target));

        changed = *module;
        changed.speed_dial.rectangular = !changed.speed_dial.rectangular;

        assert(!speed_dial_target_valid(&changed, x, y, &target));

        changed = *module;
        changed.art_id[0] = changed.art_id[0] == 'a' ? 'b' : 'a';

        assert(!speed_dial_target_valid(&changed, x, y, &target));

        changed = *module;
        changed.kind = DISPLAY_ROON;
...
int main(int argc, char **argv)
{
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        struct {
            ...
            unsigned after;
        } guarded = {.before = 0xaabbccdd, .after = 0xaabbccdd};
        ...
        if (!valid)
            ...
            printf("{\"kind\":%u,\"count\":%u,\"capacity\":%u,\"page\":%u,\"pages\":%u,\"slots\":[",
                   module->kind, module->speed_dial.count, speed_dial_layout(module, NULL),
                   module->page_index, module->page_count);

            for (unsigned i = 0; i < module->speed_dial.count; ++i) {
                speed_dial_geometry_t g;

                assert(speed_dial_geometry(module, i, &g));

                printf(
                    "%s{\"x\":%d,\"y\":%d,\"width\":%d,\"height\":%d,\"radius\":%d,\"icon\":{\"x\":%d,\"y\":%d,\"size\":%d},\"label\":",
                    i ? "," : "", g.button.x, g.button.y, g.button.width, g.button.height, g.radius,
                    g.icon.x, g.icon.y, g.icon.width);
                if (g.label.width)
                    printf("{\"x\":%d,\"y\":%d,\"width\":%d,\"height\":%d,\"align\":\"%s\"}",
                           g.label.x, g.label.y, g.label.width, g.label.height,
                           module->speed_dial.list ? "left" : "center");
                else
                    printf("null");
                printf(",\"status\":{\"x\":%d,\"y\":%d,\"size\":%d}}", g.status.x, g.status.y,
                       g.status.width);
            }
