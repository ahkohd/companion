...
async function fixture(t, options = {}) {
  ...
  const store = new FaceStore(),
    filePath = path.join(directory, 'studio.json')

  return {
    store,
    filePath,
    directory,
    settings: new StudioSettings(store, { filePath, ...options }),
  }
}
...
test('settings migrate legacy text spacing and persist display and mouse preferences together', async (t) => {
  ...
  const legacyPath = path.join(directory, 'display.json')
  await writeFile(legacyPath, '{"textGap":16}')
  const store = new FaceStore()

  await new DisplaySettings(store, { filePath: legacyPath }).load()
  const settings = new StudioSettings(store, { filePath })
  await settings.load()

  assert.equal(store.settings.device.textGap, 16)
  assert.equal(store.textGap, 16)

  await settings.save({ device: { followMouse: true, mouseInterval: 250, textGap: 4 } })
  const next = new FaceStore(),
    restarted = new StudioSettings(next, { filePath })
  await restarted.load()

  assert.equal(next.textGap, 4)
  assert.equal(next.settings.device.followMouse, true)
  assert.equal(next.settings.device.mouseInterval, 250)
...
test('removed clip mappings fall back without losing any other saved settings', async (t) => {
        ...
        buttons: [
          {
            ...
            actions: [{ type: 'url', value: 'https://example.com' }],
          },
        ],
      },
    },
  })
  saved.mappings.working = 'grok:happy'
  saved.mappings.blocked = 'grok:angry'
  saved.mappings.done = 'grok:celebrate'
  ...
  await settings.save({ appearance: { theme: 'light' } })
  expected.appearance.theme = 'light'

  assert.deepEqual(JSON.parse(await readFile(filePath, 'utf8')), expected)

  const restarted = new StudioSettings(new FaceStore(), { filePath })
  await restarted.load()
...
test('concurrent partial settings merge in order without losing unrelated changes', async (t) => {
  const { settings, store } = await fixture(t)

  await settings.load()
  await settings.save({
    appearance: { theme: 'light' },
    modules: { usage: { enabled: false }, hey: { enabled: false }, clock: { enabled: false } },
  })
  ...
  assert.equal(results[0].modules.face.enabled, true)
  assert.equal(results[1].modules.face.enabled, false)
  assert.equal(store.activeModule, 'usage')
  assert.equal(store.settings.device.activeModule, 'usage')
  assert.equal(store.settings.appearance.theme, 'dark')
  assert.equal(store.textGap, 4)
  assert.equal(store.settings.mappings.working, 'done')
  assert.equal(store.settingsRevision, revision + 5)
})
...
test('failed writes do not change live settings and the write queue recovers', async (t) => {
  ...
  await assert.rejects(settings.save({ device: { textGap: 4 } }))
  assert.equal(store.textGap, 8)
  assert.equal(store.settingsRevision, 0)

  await rm(filePath, { recursive: true })
  await settings.save({ device: { textGap: 16 } })

  assert.equal(store.textGap, 16)
  assert.equal(store.settingsRevision, 1)
})
...
test('invalid settings are rejected without writes and cannot disable every module', async (t) => {
  ...
  for (const patch of [
    ...
    { device: { activeModule: 'other' } },
    { device: { moduleOrder: ['face', 'face', 'hey'] } },
    { mappings: { working: 'grok:not-real' } },
    { modules: { usage: { refreshSeconds: 1 } } },
    { device: null },
    { appearance: { theme: 'other' } },
...
test('provider validation agrees with the collector and snapshots retain valid source settings', async (t) => {
  ...
  for (const providers of [
    ...
    ['--flag'],
    ['codex', 'codex'],
    Array.from({ length: 17 }, (_, n) => `provider-${n}`),
  ]) {
    await assert.rejects(settings.save({ modules: { usage: { providers } } }))
  }

  const valid = await settings.save({
    modules: { usage: { providers: ['codex', 'azure-openai'], provider: 'codex' } },
  })
  const source = new ModuleSources()

  assert.doesNotThrow(() => source.configure(valid.modules))
...
test('module activation and queued swipes preserve order and skip disabled modules', async (t) => {
  ...
  await settings.save({
    modules: { usage: { enabled: true }, hey: { enabled: true } },
    device: { moduleOrder: ['hey', 'face', 'usage', 'clock', 'roon', 'audio', 'speedDial'] },
  })
  await Promise.all([settings.cycleModule(1), settings.cycleModule(1)])

  assert.equal(store.activeModule, 'hey')
  assert.equal(JSON.parse(await readFile(filePath, 'utf8')).device.activeModule, 'hey')

  await settings.save({ modules: { usage: { enabled: false } } })
  await settings.cycleModule(-1)

  assert.equal(store.activeModule, 'face')

  await settings.save({ device: { swipeEnabled: false } })
  const revision = store.settingsRevision
  await settings.cycleModule(1)

  assert.equal(store.activeModule, 'face')
  assert.equal(store.settingsRevision, revision)
  await assert.rejects(settings.cycleModule(0))
  await assert.rejects(settings.activateModule('unknown'))
})

test('saved files are validated and platform checks run before committing a preference', async (t) => {
  const { settings, store, filePath } = await fixture(t, {
    validateApply: (value) => {
      if (value.device.followMouse) throw new Error('Unsupported platform')
    },
  })
  ...
  assert.equal(store.activeModule, 'face')
  await assert.rejects(settings.save({ device: { followMouse: true } }), /Unsupported/)
  assert.equal(store.settings.device.followMouse, false)

  const saved = defaultSettings()
  saved.device.followMouse = true
  await writeFile(filePath, JSON.stringify(saved))
  await settings.load()
...
test('unrelated settings and identical saves preserve animation age', async (t) => {
  ...
  const revision = store.settingsRevision
  await settings.save({ appearance: { theme: 'dark' } })
...
test('native remaps preserve logical statuses and captions', () => {
  ...
  store.ingest([{ pane_id: 'a', agent: 'pi', agent_status: 'working', name: 'Project task' }])
  store.select('a')
  store.setSettings(mergeSettings(defaultSettings(), { mappings: { working: 'sleep' } }))

  assert.equal(store.frame().state, 'working')
  assert.equal(store.frame().expression, 'sleep')
  assert.equal(store.frame().label, 'Working')
  assert.equal(store.frame().name, 'Project task')
  assert.equal(store.frame().preview, false)

  store.setSettings(mergeSettings(store.settings, { mappings: { working: 'done' } }))

  assert.equal(store.frame().expression, 'done')
  assert.equal(store.frame().state, 'working')

  store.setExpression('unknown')

  assert.equal(store.frame().state, 'unknown')
  assert.equal(store.frame().label, 'Status unknown')

  store.setExpression('disconnected')
...
test('existing settings default to hidden navigation and preserve the choice after restart', async (t) => {
  ...
  delete legacy.device.showModuleNavigation
...
test('card backgrounds default off, persist both choices and reach every module frame', async (t) => {
  const { settings, store, filePath } = await fixture(t)
  const legacy = defaultSettings()
  delete legacy.device.showCardBackgrounds
...
test('display rotation migrates upright, persists, and reaches every module frame', async (t) => {
  ...
  for (const rotation of [0, 1, 15, 37, 90, 180, 270, 359]) {
    ...
    for (const id of store.enabledModules()) {
      store.setModule(id)

      assert.equal(store.frame().rotation, rotation)
    }
  }
  ...
  for (const rotation of [-90, 45.5, 360, '90', null, true])
    await assert.rejects(settings.save({ device: { rotation } }))
