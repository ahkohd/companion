...
test('server streams rotating working sessions without attached hardware', { timeout: 12000 }, async t => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'working-session-http-'));
  const socketPath = path.join(directory, 'herdr.sock'), peers = new Set();
  const herdr = net.createServer(socket => {
    ...
    socket.on('data', chunk => {
      ...
      const result = request.method === 'agent.list'
        ? { agents: ['Alpha', 'Beta'].map(name => ({ pane_id: name, name, agent: 'pi', agent_status: 'working' })) }
        : { type: 'subscribed' };
  ...
  for (const cli of ['codexbar', 'hey']) await writeFile(path.join(directory, cli),
    `#!${process.execPath}\nconsole.log('1.0.0');\n`, { mode: 0o700 });
  const child = spawn(process.execPath, ['bridge/server.mjs'], {
    env: { ...process.env, PATH: directory + path.delimiter + process.env.PATH, PORT: '0', ESP_SERIAL_PORT: '',
      HERDR_SOCKET_PATH: socketPath, STUDIO_SETTINGS_PATH: path.join(directory, 'studio.json'), DISPLAY_SETTINGS_PATH: path.join(directory, 'display.json') },
  ...
  while (true) {
    ...
    while ((end = buffer.indexOf('\n\n')) >= 0) {
      ...
      assert.ok(initial); assert.equal(snapshot.display.label, '2 Working');
