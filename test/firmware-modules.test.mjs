...
const needsIdf = {
  skip: !parserAvailable && 'Run firmware:setup to provide the firmware cJSON dependency',
}
...
test(
  'module protocol retains legacy defaults and distinguishes missing values from zero',
  needsIdf,
  () => {
    const [legacy, absent, zero, partial] = parse(
      ...
      { module: 'usage', dashboard: { status: 'ready', primary: { remaining: null } } },
      {
        ...
        moduleIndex: 1,
        moduleCount: 3,
        dashboard: { status: 'ready', primary: { remaining: 0 }, secondary: { remaining: 100 } },
      },
      {
        module: 'hey',
        dashboard: {
          ...
          boxes: [
            { label: 'Imbox', count: 0 },
            { label: 'Feed', count: null },
          ],
        },
      },
    )

    assert.equal(legacy.kind, 0)
    assert.equal(legacy.modules, 1)
    assert.equal(legacy.status, 2)
    assert.equal(legacy.pageIndex, 0)
    assert.equal(legacy.pageCount, 1)
    assert.equal(absent.primary, false)
    assert.equal(zero.primary, true)
    assert.equal(zero.remaining, 0)
    assert.equal(zero.index, 1)
    assert.equal(zero.modules, 3)
    assert.equal(partial.hasCount, true)
    assert.equal(partial.count, 100)
    assert.equal(partial.countMore, true)
    assert.deepEqual(partial.boxAvailable, [true, false, false])
  },
)

test(
  'module parser rejects invalid kinds, positions and truncated or oversized metric data',
  needsIdf,
  () => {
    const invalid = [
      { module: 'calendar' },
      ...
      { moduleIndex: 1 },
      { moduleCount: 2 },
      { moduleIndex: 3, moduleCount: 3 },
      { moduleIndex: 0, moduleCount: 0 },
      { moduleIndex: 0, moduleCount: 8 },
      { moduleIndex: 0.5, moduleCount: 2 },
      { dashboard: null },
      ...[null, 'true', 1, []].map((showModuleNavigation) => ({ showModuleNavigation })),
      ...['missing', null, 1, true].map((status) => ({ dashboard: { status } })),
      ...[-1, 100.01, '50', true].map((remaining) => ({
        dashboard: { status: 'ready', primary: { remaining } },
      })),
...
test(
  'Roon accepts bounded UTF-8 track data, playback flags and five-module navigation',
  needsIdf,
  () => {
    const dashboard = {
      status: 'ready',
    ...
    assert.equal(
      parse({ module: 'roon', dashboard: { ...dashboard, expanded: true } })[0].expanded,
      true,
    )
    assert.equal(
      parse({ module: 'roon', dashboard: { ...dashboard, expanded: false } })[0].expanded,
      false,
    )
    assert.ok(
      parse({ module: 'roon', dashboard: { ...dashboard, artId: '', track: '', artist: '' } })[0],
    )
    assert.equal(parse({ module: 'roon', dashboard: { status: 'loading' } })[0].status, 1)

    const invalid = [
      ...[null, 1, 0, 'true', [], {}].map((expanded) => ({ ...dashboard, expanded })),
      ...['track', 'artist', 'artId', 'playing', 'canPrevious', 'canNext'].map((key) => ({
        ...dashboard,
        [key]: undefined,
      })),
      ...['track', 'artist'].flatMap((key) =>
        [null, true, 42, 'x'.repeat(65), '\u00e9'.repeat(33), 'a\nb'].map((value) => ({
          ...dashboard,
          [key]: value,
        })),
      ),
      ...['x', 'a'.repeat(39), 'a'.repeat(41), 'g'.repeat(40), null].map((artId) => ({
        ...dashboard,
        artId,
      })),
      ...['playing', 'canPrevious', 'canNext'].flatMap((key) =>
        [null, 1, 'true'].map((value) => ({ ...dashboard, [key]: value })),
      ),
...
test(
  'clock separator blinking is optional for old bridges and strictly validates its boolean',
  needsIdf,
  () => {
    const frame = (blinkSeparator) => ({
      module: 'clock',
      dashboard: { status: 'ready', time: '12:59', weekday: 'Wed', blinkSeparator },
    })

    assert.deepEqual(
      parse(...[undefined, false, true].map(frame)).map((result) => result.blinkSeparator),
      [false, false, true],
    )
...
test('usage pages require paired integer bounds and preserve valid page limits', needsIdf, () => {
  const invalid = [
    ...
    { pageIndex: 0, pageCount: 257 },
    { pageIndex: 256, pageCount: 256 },
    { pageIndex: -1, pageCount: 2 },
    { pageIndex: 0.5, pageCount: 2 },
    { pageIndex: 0, pageCount: 1.5 },
    { pageIndex: '0', pageCount: 2 },
    { pageIndex: 0, pageCount: true },
    { pageIndex: null, pageCount: 1 },
  ...
  const valid = parse(
    { module: 'usage', dashboard: { status: 'ready', pageIndex: 0, pageCount: 1 } },
    {
      ...
      dashboard: {
        ...
        pageIndex: 255,
        pageCount: 256,
        primary: { provider: '\u00e9'.repeat(8) },
      },
    },
  )

  assert.equal(valid[0].pageIndex, 0)
  assert.equal(valid[0].pageCount, 1)
  assert.equal(valid[1].pageIndex, 255)
  assert.equal(valid[1].pageCount, 256)
  assert.equal(valid[1].providerBytes, 16)
})

test(
  'module parser accepts every connection state and bounded fractional percentages',
  needsIdf,
  () => {
    ...
    const results = parse(
      ...statuses.map((status) => ({
        ...
        dashboard: {
          status,
          primary: { label: 'Session', remaining: 73.25, reset: 'Resets in 2h' },
        },
      })),
    )
    ...
    results.forEach((result, index) => {
      ...
      assert.equal(result.remaining, 73.25)
    })
  },
)

test('navigation is hidden by default without changing swipe module count', needsIdf, () => {
  const frames = [undefined, false, true].map((showModuleNavigation) => ({
    moduleIndex: 1,
    moduleCount: 3,
  ...
  assert.deepEqual(
    results.map((item) => item.navigation),
    [false, false, true],
  )
...
test('cached refresh state is optional and accepts only booleans', needsIdf, () => {
  const valid = parse(
    {},
    ...[undefined, false, true].map((refreshing) => ({
      module: 'usage',
      dashboard: { status: 'ready', refreshing },
    })),
  )

  assert.deepEqual(
    valid.map((item) => item.refreshing),
    [false, false, false, true],
  )

  const invalid = [null, 'true', 1, 0, [], {}].map((refreshing) => ({
    module: 'usage',
    dashboard: { status: 'ready', refreshing },
  }))
...
test(
  'card backgrounds default off and accept only boolean settings across modules',
  needsIdf,
  () => {
    ...
    for (const module of ['usage', 'hey']) {
      ...
      assert.deepEqual(
        parse(...frames).map((item) => item.cardBackgrounds),
        [false, true, false, true, false],
      )
    }

    const invalid = [null, 'true', 1, 0, [], {}].map((showCardBackgrounds) => ({
      showCardBackgrounds,
    }))
...
test('HEY pages preserve bounded sender and subject strings, including UTF-8', needsIdf, () => {
  ...
  assert.deepEqual(results[1].messageBytes, [
    [32, 64],
    [32, 64],
    [32, 63],
  ])
...
test(
  'HEY rows reject missing fields, invalid types, controls and oversized UTF-8',
  needsIdf,
  () => {
    const invalid = [
      null,
      ...
      ...[null, false, 42, {}, []].flatMap((value) => [
        [{ sender: value, subject: 'B' }],
        [{ sender: 'A', subject: value }],
      ]),
...
test('card targets keep bounded opaque tokens and strict optional openable flags', needsIdf, () => {
  ...
  const [usage, hey, old] = parse(
    {
      ...
      dashboard: {
        status: 'ready',
        openToken: token,
        primary: { remaining: null, openable: true },
        secondary: { remaining: 80, openable: false },
      },
    },
    {
      module: 'hey',
      dashboard: {
        ...
        openToken: token,
        items: [
          { sender: 'Alice', subject: 'Hello', openable: true },
          { sender: 'Bob', subject: 'World', openable: false },
        ],
      },
    },
  ...
  assert.equal(usage.openToken, token)
  assert.deepEqual(usage.openable, [true, false])
  assert.equal(hey.openToken, token)
  assert.deepEqual(hey.messageOpenable, [true, false, false])
  assert.equal(old.openToken, '')
  assert.deepEqual(old.openable, [false, false])
  ...
  for (const openable of [1, 0, 'true', null, {}, []]) {
    assert.equal(
      parse({
        module: 'usage',
        dashboard: { status: 'ready', primary: { remaining: 10, openable } },
      })[0],
      null,
    )
    assert.equal(
      parse({
        module: 'hey',
        dashboard: { status: 'ready', items: [{ sender: 'Alice', subject: 'Hello', openable }] },
      })[0],
      null,
    )
  }
...
test(
  'device theme validates all semantic color tokens and preserves legacy frames',
  needsIdf,
  () => {
    const palette = {
      background: 0xffffff,
    ...
    for (const key of Object.keys(palette)) {
      for (const value of [-1, 0x1000000, 1.5, '#ffffff', null, true])
        assert.equal(parse({ palette: { ...palette, [key]: value } })[0], null)

      const missing = { ...palette }
      delete missing[key]

      assert.equal(parse({ palette: missing })[0], null)
    }
...
test(
  'Now Playing sources retain navigation while unavailable and bound like flags',
  needsIdf,
  () => {
    const frames = ['roon', 'spotify', 'system', 'appleMusic'].map((player, pageIndex) => ({
      module: 'roon',
      dashboard: {
        status: 'unavailable',
        player,
        pageIndex,
        pageCount: 4,
        canLike: player === 'spotify',
        liked: player === 'spotify',
      },
    }))
    const results = parse(...frames)

    results.forEach((result, index) => {
      assert.equal(result.player, index)
      assert.equal(result.pageCount, 4)
      assert.equal(result.pageIndex, index)
      assert.equal(result.canLike, index === 1)
      assert.equal(result.liked, index === 1)
    })
    const legacy = parse({ module: 'roon', dashboard: { status: 'loading' } })[0]

    assert.equal(legacy.player, 0)
    assert.equal(legacy.canLike, false)
    assert.equal(legacy.liked, false)
    ...
    assert.deepEqual(
      parse(
        ...invalid.map((fields) => ({
          module: 'roon',
          dashboard: { status: 'loading', ...fields },
        })),
      ),
      invalid.map(() => null),
    )
...
test('Audio preserves input/output device targets and rejects malformed controls', needsIdf, () => {
  ...
  assert.equal(result.audioDevice, 17)
  assert.equal(result.audioNextDevice, 42)
  assert.equal(result.audioDeviceCount, 2)
  assert.equal(result.audioInput, 0)
  assert.equal(result.audioVolume, 52.5)
  assert.equal(result.audioCanVolume, 1)
  assert.equal(result.audioCanMute, 1)

  const input = parse({
    module: 'audio',
    dashboard: { ...dashboard, scope: 'input', pageIndex: 1, muted: true },
  })[0]

  assert.equal(input.audioInput, 1)
  assert.equal(input.audioMuted, 1)

  const unavailable = parse({
    module: 'audio',
    dashboard: { ...dashboard, status: 'unavailable', deviceId: 0, volume: null, muted: null },
  })[0]

  assert.equal(unavailable.pageCount, 2)
  assert.equal(unavailable.audioCanVolume, 0)
  assert.equal(unavailable.audioCanMute, 0)

  const invalid = [
    { nextDeviceId: -1 },
    { nextDeviceId: 0x100000000 },
    { deviceCount: 1.5 },
    { scope: 'other' },
    { scope: null },
    { deviceId: -1 },
    { deviceId: 1.5 },
    { deviceId: 0x100000000 },
  ...
  assert.deepEqual(
    parse(
      ...invalid.map((fields) => ({ module: 'audio', dashboard: { ...dashboard, ...fields } })),
    ),
    invalid.map(() => null),
  )
})
...
test('Audio picker accepts visible device pages and rejects malformed rows', needsIdf, () => {
  const dashboard = {
    ...
    pageCount: 3,
    devices: [
      { id: 17, name: 'Speakers', active: true },
      { id: 42, name: 'External DAC', active: false },
    ],
  }
  const value = parse({ module: 'audio', dashboard })[0]

  assert.equal(value.audioPicker, 1)
  assert.equal(value.audioRows, 2)
  assert.deepEqual(value.audioRowIDs, [17, 42, 0])
  assert.equal(value.pageIndex, 2)

  const invalid = [
    { pickerOpen: 'true' },
    { devices: null },
    { devices: Array(4).fill(dashboard.devices[0]) },
    { devices: [{ id: 0, name: 'Bad', active: false }] },
    { devices: [{ id: 42, name: 'x'.repeat(65), active: false }] },
    { devices: [{ id: 42, name: 'Bad', active: 1 }] },
    { devices: [dashboard.devices[0], dashboard.devices[0]] },
  ]
  ...
  assert.equal(parse({ module: 'audio', dashboard: missing })[0], null)
})
