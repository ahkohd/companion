...
int main(void)
{
    ...
    for (int height = 2; height <= 24; ++height) for (int style = 1; style <= 2; ++style)
    for (int size = 2; size <= 16; ++size) for (int gap = 0; gap <= 8; gap++) {
        int width = widths[w];
        check(width, height, style, size, gap, (cases % 3 == 0) ? 0 : (cases % 3 == 1) ? width : width / 3 + 1);
        cases++;
    }
    for (int fill = 0; fill <= 100; ++fill) for (int style = 1; style <= 2; ++style) {
        check(100, 10, style, 4, 2, fill); cases++;
    }
    for (int invalid = -1; invalid <= 4; ++invalid) if (invalid != 1 && invalid != 2)
    ...
    printf("PASS: %u patterned bar cases validate full-cell centering, transparent gaps, square/circle coverage, exact clipped progress, all/none fill and buffer guards\n", cases);
}
