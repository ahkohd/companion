...
before(async () => {
  ...
  if (available) {
    ...
    const type = source.slice(
      source.indexOf('typedef struct {'),
      source.indexOf('static SemaphoreHandle_t'),
    )
    const functions = source
      .slice(
        source.indexOf('static bool json_uint('),
        source.indexOf('static bool name_shimmer_enabled('),
      )
      .replace(/static int status_top_for_gap\([^]*?\n}\n/, '')
    const definitions = source
      .match(/^#define (?:FRAME_MAX|FACE_LABEL_CAPACITY|FACE_NAME_CAPACITY|TEXT_GAP_DEFAULT) .*$/gm)
      .join('\n')
    ...
    const sourcePath = path.join(directory, 'frames.c')
    frameProbe = path.join(directory, 'frames')
    ...
    execFileSync('cc', [
      ...
      sourcePath,
      'firmware/main/display_module.c',
      'firmware/main/attention_protocol.c',
      path.join(cjson, 'cJSON.c'),
      '-lm',
      '-o',
      frameProbe,
...
const parse = (frames) =>
  execFileSync(probe, [], { input: frames.map(JSON.stringify).join('\n') + '\n', encoding: 'utf8' })
    .trim()
    .split('\n')
    .map(JSON.parse)
...
test(
  'native design schema matches the shared schema and omitted arrays use exact defaults',
  needsIdf,
  () => {
    ...
    assert.deepEqual(
      parse(modules.map((module) => ({ module }))),
      modules.map((module) => ({ module, values: defaults(module) })),
    )
    assert.deepEqual(parse([{}])[0], { module: 'face', values: defaults('face') })
    assert.deepEqual(parse([{ module: 'face', design: defaults('face').slice(0, 8) }])[0], {
      module: 'face',
      values: defaults('face'),
    })

    const legacyUsage = defaults('usage').slice(0, 25)
    legacyUsage[schema.usage.findIndex((field) => field.key === 'barHeight')] = 20

    assert.deepEqual(parse([{ module: 'usage', design: legacyUsage }])[0], {
      module: 'usage',
      values: [...legacyUsage, ...defaults('usage').slice(25)],
    })

    const legacyRoon = defaults('roon').slice(0, 13)
    legacyRoon[schema.roon.findIndex((field) => field.key === 'controlSize')] = 28

    assert.deepEqual(parse([{ module: 'roon', design: legacyRoon }])[0], {
      module: 'roon',
      values: [...legacyRoon, 0, 0],
    })
  },
)

test(
  'every design option and range boundary reaches native fields in the shared order',
  needsIdf,
  () => {
    const frames = [],
      expected = []
    ...
    for (const [module, fields] of Object.entries(schema))
      fields.forEach((field, index) => {
        for (const value of [
          ...(field.options || [field.min, field.max]),
          ...(field.allowAuto ? [0] : []),
        ]) {
          const values = defaults(module)
          values[index] = value
          frames.push({ module, design: values })
          expected.push({ module, values })
        }
      })
...
test(
  'native rejects partial arrays, invalid numeric fields and unsupported options',
  needsIdf,
  () => {
    const frames = []
    ...
    for (const [module, fields] of Object.entries(schema)) {
      for (const design of [
        ...
        'default',
        [],
        defaults(module).slice(1),
        [...defaults(module), 0],
      ])
        frames.push({ module, design })
      ...
      fields.forEach((field, index) => {
        ...
        if (field.options)
          invalid.push(
            ...[...Array(field.max + 1).keys()]
              .filter((value) => !field.options.includes(value))
              .slice(0, 2),
          )

        for (const value of invalid) {
          const design = defaults(module)
          design[index] = value
          frames.push({ module, design })
        }
      })
...
test(
  'production state parser accepts 4096-byte designer frames and rejects overflow',
  needsIdf,
  () => {
    const frame = {
      type: 'state',
      ...
      design: defaults('face'),
      padding: '',
    }
    const base = JSON.stringify(frame).length
    const inputs = [2048, 2049, 4096, 4097].map((bytes) =>
      JSON.stringify({ ...frame, padding: 'x'.repeat(bytes - base) }),
    )
    const results = execFileSync(frameProbe, [], {
      input: inputs.join('\n') + '\n',
      encoding: 'utf8',
    })
      .trim()
...
test('production state parser rejects removed clips and accepts native overrides', needsIdf, () => {
  ...
  const valid = [
    ...
    ...['working', 'blocked', 'done', 'idle', 'sleep', 'unknown', 'disconnected'].map(
      (expression) => ({ ...frame, expression }),
    ),
  ]
  const invalid = ['grok:happy', 'grok:celebrate', 'done', '', true, 1, [], {}].map(
    (animation) => ({ ...frame, animation }),
  )
  const result = execFileSync(frameProbe, [], {
    input: [...valid, ...invalid].map(JSON.stringify).join('\n') + '\n',
    encoding: 'utf8',
  })
    .trim()
    .split('\n')
...
test(
  'production state parser accepts every integer rotation and retains legacy frames',
  needsIdf,
  () => {
    const frame = {
      type: 'state',
    ...
    const invalid = [-90, -1, 360, 0.5, 180.5, true, null, '90', {}, []].map((rotation) => ({
      ...frame,
      rotation,
    }))
    const results = execFileSync(frameProbe, [], {
...
test('production attention parser validates bounded actions and descriptions', needsIdf, () => {
  ...
  const cases = [
    undefined,
    attention,
    { ...attention, detail: false },
    { ...attention, body: 'x'.repeat(481) },
    { ...attention, revision: -1 },
    { ...attention, revision: 1.5 },
    { ...attention, actions: [] },
  ...
  const result = execFileSync(frameProbe, [], {
    input: cases.map((attention) => JSON.stringify({ ...frame, attention })).join('\n') + '\n',
    encoding: 'utf8',
  })
    .trim()
    .split('\n')
...
test('attention double tap never emits first approval and cancels stale gestures', needsIdf, () => {
  ...
  execFileSync('cc', [
    ...
    '-I',
    cjson,
    'test/attention-gesture-probe.c',
    'firmware/main/attention_gesture.c',
    '-o',
    gestureProbe,
  ])
...
test(
  'production frame boundary rejects escaped NUL before cJSON can truncate identifiers',
  needsIdf,
  () => {
    const frame = {
      type: 'state',
