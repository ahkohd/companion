...
test('HTTP and SSE serve real state, restrict writes and shut down cleanly', {
  timeout: 15000,
}, async (t) => {
    ...
    socket.on('data', (chunk) => {
      ...
      const result =
        req.method === 'agent.list'
          ? { agents: [{ pane_id: 'a', agent: 'pi', name: 'Test agent', agent_status: 'blocked' }] }
          : { type: 'subscribed' }
  ...
  const logResponse = await fetch(base + '/api/logs')

  assert.equal(logResponse.status, 200)

  const diagnostics = await logResponse.json()

  assert.equal(typeof diagnostics.sessionId, 'string')
  assert.ok(diagnostics.entries.some((entry) => entry.message.startsWith('Companion:')))
  ...
  const install = (value, headers = {}) =>
    fetch(base + '/api/installations', {
      method: 'POST',
  ...
  assert.equal(
    (
      await install(
        { action: 'skills.install', name: 'companion-attention' },
  ...
  assert.equal(
    (await install({ action: 'cli.install' }, { 'Content-Type': 'text/plain' })).status,
    415,
  )
  ...
  while (!streamText.includes('Test agent'))
    streamText += new TextDecoder().decode((await reader.read()).value)

  assert.match(streamText, /"state":"blocked"/)

  await reader.cancel()

  const request = (body, headers = {}) =>
    fetch(base + '/api/select', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
      body,
    })

  const selected = await request('{"id":"a"}')

  assert.equal(selected.status, 200)
  assert.equal((await selected.json()).selected, 'a')
  ...
  const usb = (route, value, headers = {}) =>
    fetch(base + '/api/device/' + route, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
      body: JSON.stringify(value),
    })
  ...
  assert.equal(
    (await usb('connection', { mode: 'manual', path: '/not-a-serial-device' })).status,
    400,
  )
  assert.equal((await usb('connection', { mode: 'invalid' })).status, 400)
  assert.equal(
    (await usb('connection', { mode: 'auto' }, { Origin: 'https://example.com' })).status,
    403,
  )
  assert.equal((await usb('reconnect', {}, { Origin: 'https://example.com' })).status, 403)
  assert.equal(
    JSON.parse(await readFile(path.join(directory, 'device-connection.json'), 'utf8')).mode,
    'off',
  )

  const spacing = (textGap, headers = {}) =>
    fetch(base + '/api/display', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
      body: JSON.stringify({ textGap }),
    })

  const close = await spacing(4)
  ...
  const expression = (value, headers = {}) =>
    fetch(base + '/api/expression', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
  ...
  for (const value of ['working', 'blocked', 'done', 'idle', 'sleep']) {
    const response = await expression(value)

    assert.equal(response.status, 200)

    const snapshot = await response.json()

    assert.equal(snapshot.expression, value)
    assert.equal(snapshot.display.state, value)
  ...
  assert.equal(current.expression, 'sleep')

  const restored = await (await expression(null)).json()

  assert.equal(restored.display.state, 'blocked')
  assert.equal(restored.expression, null)
  assert.equal(restored.selected, 'a')

  const pointer = (value, headers = {}) =>
    fetch(base + '/api/pointer', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
      body: JSON.stringify(value),
    })
  ...
  assert.equal(
    (await pointer({ enabled: true, intervalMs: 100 }, { Origin: 'https://example.com' })).status,
    403,
  )

  const pointerState = await (await fetch(base + '/api/state')).json()

  assert.equal(pointerState.pointer.enabled, false)
  assert.equal(pointerState.pointer.intervalMs, 250)

  await expression('done')

  assert.equal((await (await request('{"id":"all"}')).json()).expression, null)

  const post = (endpoint, value, headers = {}) =>
    fetch(base + endpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers },
      body: JSON.stringify(value),
  ...
  const attentionShow = await post('/api/attention/show', {
    owner: 'test-cli',
    kind: 'decision',
    title: 'Approve release?',
    body: 'Review the changes first.',
  })

  assert.equal(attentionShow.status, 200)

  const attentionShown = await attentionShow.json(),
    attentionId = attentionShown.attentionResult.id

  assert.equal(attentionShown.attention.active.id, attentionId)
  assert.equal(attentionShown.display.label, 'Approve release?')
  assert.equal(
    (await post('/api/attention/act', { id: attentionId, revision: 1, action: 'approve' })).status,
    400,
  )
  assert.equal(
    (await post('/api/attention/clear', { id: attentionId, owner: 'different' })).status,
    400,
  )
  assert.equal(
    (
  ...
  const details = await (
    await post('/api/attention/details', { id: attentionId, revision: 1, detail: true })
  ).json()
  const responded = await (
    await post('/api/attention/act', {
      id: attentionId,
      revision: details.attention.active.revision,
      action: 'approve',
    })
  ).json()

  assert.equal(responded.attention.active, null)
  assert.equal(responded.attention.history[0].action, 'approve')
  assert.equal(
    (
      await post('/api/attention/act', {
        id: attentionId,
  ...
  const dismissible = await (
    await post('/api/attention/show', {
      owner: 'test-cli',
      kind: 'decision',
      title: 'Gesture test',
    })
  ...
  const gestureTarget = {
    id: dismissible.attention.active.id,
    revision: dismissible.attention.active.revision,
  }

  assert.equal(
    (await post('/api/attention/dismiss', gestureTarget, { Origin: 'https://example.com' })).status,
    403,
  )
  ...
  assert.equal(
    (await post('/api/open-card', { module: 'hey', index: 0, token: 'a'.repeat(40) })).status,
    400,
  )
  ...
  assert.match((await invalidView.json()).error, /on or off/)

  const unavailableView = await post('/api/roon/view', { expanded: true })

  assert.equal(unavailableView.status, 400)
  assert.match((await unavailableView.json()).error, /Show Roon/)
  ...
  assert.equal(detection.modules.hey.installed, true)
  assert.equal(detection.modules.hey.status, 'disabled')
  ...
  assert.equal(studioState.settings.device.textGap, 16)
  assert.equal(studioState.settings.device.mouseInterval, 250)
  assert.equal(studioState.settings.modules.usage.enabled, true)
  assert.equal(studioState.settings.mappings.working, 'done')

  const usageModule = await (await post('/api/module', { id: 'usage' })).json()

  assert.equal(usageModule.module, 'usage')

  const usage = await (await post('/api/modules/refresh', { id: 'usage' })).json()

  assert.equal(usage.display.dashboard.pageCount, 3)
  assert.equal(usage.display.dashboard.pageIndex, 0)

  const settingsBeforePages = await readFile(studioPath, 'utf8')
  const pages = await Promise.all([
    post('/api/usage/page', { direction: 1 }),
    post('/api/usage/page', { direction: 1 }),
  ])
  ...
  assert.equal(paged.display.dashboard.pageIndex, 2)
  assert.equal(paged.display.dashboard.primary.provider, 'Claude')
  assert.equal(paged.display.dashboard.primary.label, 'Weekly')
  assert.equal(paged.display.dashboard.secondary, undefined)
  assert.equal(paged.settingsRevision, usage.settingsRevision)
  assert.equal(await readFile(studioPath, 'utf8'), settingsBeforePages)
  assert.equal(
    (await (await post('/api/usage/page', { direction: 1 })).json()).display.dashboard.pageIndex,
    0,
  )

  for (const direction of [undefined, null, '1', 0, 2, -2])
    assert.equal((await post('/api/usage/page', { direction })).status, 400)

  assert.equal(
    (await post('/api/usage/page', { direction: 1, padding: 'x'.repeat(1024) })).status,
    413,
  )
  ...
  assert.equal(filtered.display.dashboard.pageCount, 1)
  assert.equal(filtered.display.dashboard.primary.provider, 'Claude')
  assert.equal(filtered.modules.usage.providers.length, 2)
  assert.equal((await post('/api/usage/page', { direction: 1 })).status, 400)

  await change({ modules: { usage: { provider: 'auto' } } })
  const selectedFace = await (await expression('working')).json()

  assert.equal(selectedFace.module, 'face')
  assert.equal(selectedFace.expression, 'working')
  assert.equal(selectedFace.display.animation, undefined)

  await post('/api/module', { id: 'hey' })
  const mailbox = await (await post('/api/modules/refresh', { id: 'hey' })).json()

  assert.equal(mailbox.display.dashboard.pageCount, 4)
  assert.equal(mailbox.display.dashboard.items[0].sender, 'Sender 1')

  const settingsBeforeMailPages = await readFile(studioPath, 'utf8')
  const mailPages = await Promise.all([
    post('/api/hey/page', { direction: 1 }),
    post('/api/hey/page', { direction: 1 }),
  ])
  ...
  assert.equal(mailPaged.display.dashboard.pageIndex, 2)
  assert.deepEqual(mailPaged.display.dashboard.items, [
    { sender: 'Sender 5', subject: 'Subject 5' },
    { sender: 'Sender 6', subject: 'Subject 6' },
  ])
  ...
  assert.equal(
    (await (await post('/api/hey/page', { direction: 1 })).json()).display.dashboard.pageIndex,
    0,
  )

  for (const direction of [undefined, null, '1', 0, 2, -2])
    assert.equal((await post('/api/hey/page', { direction })).status, 400)
  ...
  assert.equal((await expression('working')).status, 400)
  assert.equal((await request('{"id":"all"}')).status, 400)
  assert.equal((await (await fetch(base + '/api/state')).json()).module, 'usage')
  assert.equal(
    (await change({ modules: { usage: { enabled: false }, hey: { enabled: false } } })).status,
    400,
  )

  for (const invalid of [
    { mappings: { working: 'grok:missing' } },
    { modules: { usage: { providers: ['bad_id'] } } },
    { device: { moduleOrder: ['face', 'face', 'hey'] } },
    { unexpected: true },
  ]) {
  ...
  assert.equal(persisted.device.textGap, 16)
  assert.equal(persisted.device.followMouse, false)
  assert.equal(persisted.device.mouseInterval, 250)
  assert.equal(persisted.device.activeModule, 'usage')
  assert.equal(persisted.mappings.working, 'done')
...
test('clock-only HTTP settings persist and SSE advances without Herdr or an attached device', {
  timeout: 10000,
}, async (t) => {
  ...
  await writeFile(
    path.join(directory, 'device-connection.json'),
    JSON.stringify({ mode: 'off', path: '', serialNumber: '' }),
  )
  const preload = path.join(directory, 'clock-time.mjs')
  await writeFile(
    preload,
    'const start = performance.now(); const base = new Date(2026, 8, 9, 23, 59).getTime(); Date.now = () => base + Math.floor((performance.now() - start) / 2000) * 60000;\n',
  )

  for (const command of ['codexbar', 'hey'])
    await writeFile(
  ...
  assert.equal(initial.module, 'clock')
  assert.equal(initial.connected, false)
  assert.equal(initial.device.status, 'disabled')
  assert.equal(initial.display.dashboard.time, '11:59')
  assert.equal(initial.display.dashboard.weekday, 'Wed')
  ...
  assert.equal(updated.module, 'clock')
  assert.equal(updated.display.dashboard.time, '00:00')
  assert.equal(updated.display.dashboard.weekday, '')
  assert.deepEqual(updated.settings.modules.clock, {
    enabled: true,
    hourFormat: '24',
    showWeekday: false,
    blinkSeparator: true,
