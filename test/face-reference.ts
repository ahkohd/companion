...
export function reference(state:FaceState) {
  ...
  let expression=EXPRESSION_BY_ID.get(({working:'curieux',unknown:'confus',sleep:'somnolent',disconnected:'somnolent'} as Record<string,string>)[state] ?? '') ?? null
  if(expression && ['sleep','disconnected'].includes(state)) expression={...expression,eyes:expression.eyes.map(e=>({...e,w:.4,h:.07,open:1})) as typeof expression.eyes}
