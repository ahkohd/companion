...
test('gaze remains finite when resuming animation after direct mouse updates', async () => {
  const result = await build({ configFile: false, logLevel: 'silent', build: {
    write: false, minify: false, lib: { entry: 'web/vendor/bloub/engine.ts', formats: ['es'] }
  } });
  const bundle = Array.isArray(result) ? result[0] : result;
  const code = bundle.output.find(item => item.type === 'chunk' && item.isEntry).code;
  const { BotEngine } = await import('data:text/javascript;base64,' + Buffer.from(code).toString('base64'));
  ...
  for (const elapsed of [0, 0.016, 0.08, 0.17, 0.2]) {
    ...
    for (const eye of frame.eyes) assert.doesNotMatch(eye.matrix + eye.d, /NaN|Infinity/, `elapsed=${elapsed}`);
  }
});
