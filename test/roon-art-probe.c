...
int main(void)
{
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if (!handled)
            ...
            printf(
                "{\"type\":\"artAck\",\"v\":1,\"transfer\":%u,\"op\":\"%s\",\"ok\":%s,\"offset\":%u,\"id\":\"%s\",\"crc32\":%u,\"revision\":%u,\"active\":%s}\n",
                ack.transfer, ack.op, ack.ok ? "true" : "false", ack.offset, art.id,
                roon_art_crc32(art.pixels, ROON_ART_BYTES), art.revision,
                art.active ? "true" : "false");
        cJSON_Delete(root);
