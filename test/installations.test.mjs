...
test('skills install offline, update, preserve edits and uninstall', async t => {
  ...
  await assert.rejects(service.change({ action: 'skills.install', name: 'companion-attention' }), /Preserving modified/);
  await assert.rejects(service.change({ action: 'skills.uninstall', name: 'companion-attention' }), /Preserving modified/);
...
test('custom directory persists and target symlinks/unmanaged files are preserved', async t => {
  ...
  await assert.rejects(service.change({ action: 'skills.uninstall', name: 'companion-attention' }), /Preserving conflict/);
  ...
  assert.equal((await reopened.change({ action: 'skills.installAll' })).skills.items[0].status, 'installed');
  await writeFile(join(real, 'companion-attention', 'personal.txt'), 'keep');
  await assert.rejects(reopened.change({ action: 'skills.uninstall', name: 'companion-attention' }), /Preserving modified/);
});
...
test('skills CLI list, path, install and uninstall run without bridge', async t => {
  const { service, home } = await fixture(t);
  const output = [];
  const deps = { installations: service, out: x => output.push(x), fetcher: () => { throw new Error('must stay offline'); } };
  ...
  await assert.rejects(service.change({ action: 'skills.directory', directory: 'relative' }), /absolute/);
});
test('empty directories count as modifications and malformed ownership is rejected', async t => {
  ...
  assert.equal((await service.snapshot()).skills.items[0].status, 'modified');
  await assert.rejects(service.change({ action: 'skills.uninstall', name: 'companion-attention' }), /modified/);
...
test('failed settings write rolls an update back to working installed files', async t => {
  ...
  service.save = async () => { throw new Error('disk write failed'); };
...
test('recovers a crashed installer lock without taking over an active installer', async t => {
  ...
  assert.equal((await service.change({ action: 'skills.installAll' })).skills.items[0].status, 'installed');
  await mkdir(lock);
  await writeFile(join(lock, 'owner.json'), JSON.stringify({ pid: process.pid }));
  await assert.rejects(service.change({ action: 'skills.uninstall', name: 'companion-attention' }), /in progress/);
});
