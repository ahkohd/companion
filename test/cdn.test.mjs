...
function fixture() {
  ...
  const object = () => ({
    size: 7,
    httpEtag: '"version"',
    uploaded: new Date('2026-09-08T00:00:00Z'),
  ...
  const env = {
    COMPANION: {
      async get(key) {
        ...
        return { ...object(), body: new Response('release').body }
      },
  ...
  const fetch = (path, init) =>
    worker.fetch(new Request(`https://cdn.victor.computer${path}`, init), env)
...
test('CDN HEAD reads metadata only and matching ETags return 304', async () => {
  ...
  const cached = await fetch('/companion/archive.zip', {
