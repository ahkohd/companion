...
static uint16_t source[SCREEN_SIDE * SCREEN_SIDE], output[SCREEN_SIDE * SCREEN_SIDE + 2],
    screen[SCREEN_SIDE * SCREEN_SIDE];
...
{
    return (uint16_t)((x * 7919) ^ (y * 1049));
}

static uint16_t swap565(uint16_t value)
{
...
static void paint(const uint16_t *pixels, screen_rect_t area)
{
    ...
    assert(area.x >= 0 && area.y >= 0 && area.x + area.width <= SCREEN_SIDE &&
           area.y + area.height <= SCREEN_SIDE);

    for (int row = 0; row < area.height; ++row)
        memcpy(screen + (area.y + row) * SCREEN_SIDE + area.x, pixels + row * area.width,
               area.width * sizeof(uint16_t));
}

static void arbitrary_updates(void)
{
    const unsigned angles[] = {1,   7,   15,  37,  45,  73,  85,  89,  91,
                               127, 179, 181, 225, 269, 271, 315, 347, 359};
    ...
    for (unsigned n = 0; n < sizeof(angles) / sizeof(*angles); ++n) {
        ...
        for (int y = 0; y < SCREEN_SIDE; ++y)
            for (int x = 0; x < SCREEN_SIDE; ++x)
                backing[y * SCREEN_SIDE + x] = pattern(x, y);

        assert(screen_rotate_backing(&transform, backing, full, output + 1,
                                     SCREEN_SIDE * SCREEN_SIDE, &area));
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            if (x + width > SCREEN_SIDE)
                width = SCREEN_SIDE - x;
            if (y + height > SCREEN_SIDE)
                height = SCREEN_SIDE - y;
            screen_rect_t dirty = {x, y, width, height};

            for (int j = 0; j < width * height; ++j)
                source[j] = (uint16_t)(0x1234 ^ (i * 173 + j * 59));

            assert(screen_backing_update(backing, source, dirty));

            output[0] = output[SCREEN_SIDE * SCREEN_SIDE + 1] = 0xa55a;

            assert(screen_rotate_backing(&transform, backing, dirty, output + 1,
                                         SCREEN_SIDE * SCREEN_SIDE, &area));
            ...
            assert(screen_rotate_backing(&transform, backing, full, reference,
                                         SCREEN_SIDE * SCREEN_SIDE, &area));
    ...
    assert(screen_rotate_backing(&transform, backing, (screen_rect_t){464, 0, 2, 2}, output, 4,
                                 &area));
...
static void changed_updates(void)
{
    ...
    for (int i = 0; i < 250; ++i) {
        ...
        for (int row = 0; row < 64; ++row)
            assert(!memcmp(source + row * 64, backing + (y + row) * SCREEN_SIDE + x,
                           64 * sizeof(uint16_t)));
    }
...
static void arbitrary_quality(void)
{
    ...
    for (int y = 0; y < SCREEN_SIDE; ++y)
        for (int x = 0; x < SCREEN_SIDE; ++x)
            backing[y * SCREEN_SIDE + x] = swap565(
                (uint16_t)((((x / 11) % 32) << 11) | (((y / 7) % 64) << 5) | ((x + y) / 17 % 32)));
    unsigned filtered = 0;
    ...
    for (unsigned n = 0; n < sizeof(angles) / sizeof(*angles); ++n) {
        ...
        assert(screen_rotate_backing(&transform, backing,
                                     (screen_rect_t){0, 0, SCREEN_SIDE, SCREEN_SIDE}, output,
                                     SCREEN_SIDE * SCREEN_SIDE, &area));

        double angle = angles[n] * 3.14159265358979323846 / 180.0;

        for (int y = 3; y < SCREEN_SIDE - 3; y += 3)
            for (int x = 3; x < SCREEN_SIDE - 3; x += 3) {
                ...
                if (ix < 0 || ix + 1 >= SCREEN_SIDE || iy < 0 || iy + 1 >= SCREEN_SIDE)
                    continue;
                ...
                for (unsigned channel = 0; channel < 3; ++channel) {
                    ...
                    for (int dy = 0; dy < 2; ++dy)
                        for (int dx = 0; dx < 2; ++dx)
                            expected +=
                                ((swap565(backing[(iy + dy) * SCREEN_SIDE + ix + dx]) >> shift) &
                                 mask) *
                ...
                if (output[y * SCREEN_SIDE + x] !=
                    backing[(int)lround(sy) * SCREEN_SIDE + (int)lround(sx)])
                    filtered++;
            }
    }
...
static void arbitrary_touch(void)
{
    for (unsigned angle = 0; angle < 360; ++angle) {
        ...
        const int points[][2] = {{233, 233}, {300, 200}, {200, 200}, {200, 300},
                                 {157, 390}, {233, 1},   {1, 233}};
        int mapped[7][2];
        ...
        assert(module_touch_end(&touch, mapped[2][0], mapped[2][1], 100000) ==
               MODULE_TOUCH_PAGE_NEXT);
...
int main(void)
{
    ...
    for (unsigned rotation = 0; rotation <= 270; rotation += 90) {
        ...
        for (int y = 0; y < SCREEN_SIDE; y += 50) {
            ...
            assert(screen_rotate_pixels(rotation, source,
                                        (screen_rect_t){0, y, SCREEN_SIDE, height}, output + 1,
                                        SCREEN_SIDE * height, &area));
            assert(output[0] == 0xa55a && output[SCREEN_SIDE * height + 1] == 0xa55a);

            for (int row = 0; row < area.height; ++row)
                memcpy(screen + (area.y + row) * SCREEN_SIDE + area.x,
                       output + 1 + row * area.width, area.width * sizeof(uint16_t));
        }
        ...
        module_touch_t touch;
        int px, py, x, y;
        forward(rotation, 300, 200, &px, &py);
        screen_unrotate_point(rotation, px, py, &x, &y);
        module_touch_begin(&touch, x, y, 0);
        forward(rotation, 200, 200, &px, &py);
        screen_unrotate_point(rotation, px, py, &x, &y);

        assert(module_touch_end(&touch, x, y, 100000) == MODULE_TOUCH_NEXT);

        forward(rotation, 200, 300, &px, &py);
        screen_unrotate_point(rotation, px, py, &x, &y);
        module_touch_begin(&touch, x, y, 0);
        forward(rotation, 200, 200, &px, &py);
        screen_unrotate_point(rotation, px, py, &x, &y);
        ...
        forward(rotation, 157, 390, &px, &py);
        screen_unrotate_point(rotation, px, py, &x, &y);
        ...
        for (int i = 0; i < 500; ++i) {
            ...
            for (int row = 0; row < height; ++row)
                for (int col = 0; col < width; ++col)
                    source[row * width + col] = pattern(x + col, y + row);
            output[0] = output[count + 1] = 0xa55a;

            assert(screen_rotate_pixels(rotation, source, (screen_rect_t){x, y, width, height},
                                        output + 1, count, &result));

            for (int row = 0; row < height; ++row)
                for (int col = 0; col < width; ++col) {
                    int px, py;
                    forward(rotation, x + col, y + row, &px, &py);

                    assert(output[1 + (py - result.y) * result.width + px - result.x] ==
                           pattern(x + col, y + row));
                }

            assert(output[0] == 0xa55a && output[count + 1] == 0xa55a);
            assert(!screen_rotate_pixels(rotation, source, (screen_rect_t){x, y, width, height},
                                         output + 1, count - 1, &result));
        }
    ...
    int x, y;
    screen_unrotate_point(0, 466, -1, &x, &y);
    ...
    changed_updates();
    printf(
        "PASS: %u complete-frame pixel/touch mappings,2000 arbitrary partial rectangles,swipes,controls,edge clamping and buffer guards\n",
        cases);
    puts(
        "PASS: 360 filtered dirty updates equal complete redraws; independent bilinear reference and all 360 touch angles");
}
